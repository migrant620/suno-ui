...
export async function recoverDraftTransaction() {
    ...
    if (journal?.version !== 1 || !records(journal.before) || !records(journal.after))
        throw new Error('Your previous draft could not be recovered. Keep this session and retry.');
    if (journal.cleanup && ![journal.cleanup.before, journal.cleanup.after].every(value => value === null || typeof value === 'string'))
        throw new Error('Your attachment cleanup could not be recovered. Keep this session and retry.');
    for (const key of DRAFT_KEYS) {
        const current = await Storage.getItem(key);
        if (current !== journal.before[key])
            await apply(key, journal.before[key]);
    }
    if (journal.cleanup && await Storage.getItem(DRAFT_CLEANUP_KEY) !== journal.cleanup.before)
        await apply(DRAFT_CLEANUP_KEY, journal.cleanup.before);
...
export async function commitDraftTransaction(after: DraftRecords, expected: DraftRecords, isActive: () => boolean, cleanup?: CleanupChange) {
    ...
    if (cleanup && await Storage.getItem(DRAFT_CLEANUP_KEY) !== cleanup.before)
        throw new Error('Attachment cleanup changed. Try Remix again.');
    check();
    try {
        ...
        for (const key of DRAFT_KEYS) {
            ...
            if (before[key] !== after[key])
                await apply(key, after[key]);
        }
        check();
        if (cleanup && cleanup.before !== cleanup.after)
            await apply(DRAFT_CLEANUP_KEY, cleanup.after);
        check();
        await Storage.removeItem(DRAFT_TRANSACTION_KEY);
    }
    catch (error) {
        if (await Storage.getItem(DRAFT_TRANSACTION_KEY) === null) {
            ...
            if (committed.every(Boolean) && (!cleanup || await Storage.getItem(DRAFT_CLEANUP_KEY) === cleanup.after))
                return;
        }
