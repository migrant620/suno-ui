...
export function useCreditPromotion() {
            ...
            try {
                ...
                if (raw === null) {
                    ...
                    try {
                        ...
                        if (!stored || !Number.isSafeInteger(stored.expiresAt) || stored.expiresAt <= 0)
                            throw new Error('Invalid offer clock');
    ...
    return { active: expiresAt !== null && expiresAt > now, label: expiresAt === null ? '' : promotionTime(expiresAt, now), error, invalid, loading, retry: () => { if (invalid && !resetting.current) {
