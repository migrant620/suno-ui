...
export function AddToPlaylist({ track, library, onClose }: {
    track: AudioTrack;
    ...
    {creating && <PlaylistForm onClose={() => setCreating(false)} onSave={(name, isPublic) => { const id = library.create(name, isPublic); if (id) {
        library.addSong(id, track.id);
        setCreating(false);
    } }}/>}
  </Sheet>;
}
