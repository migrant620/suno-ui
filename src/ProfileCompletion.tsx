...
export function ProfileCompletion({ account, onClose, onContinue, recovery }: { account: ReturnType<typeof useLocalProfile>; onClose: () => void; onContinue: () => void; recovery?: ProfilePickerSession | null }) {
  ...
  const choose = async (camera: boolean) => {
    if (pending.current || !alive.current || !account.ready) return;
    ...
    try {
      ...
      if (camera && Platform.OS !== 'web') {
        ...
        if (!active()) return;
        if (!permission.granted) { setError('Camera access was not granted. Choose from your library or allow access in device settings.'); return; }
      }
      ...
      if (!result.canceled && active()) {
        ...
        if (active()) setPhoto(uri);
      }
    } catch { if (active()) setError('This image could not be opened. Try again or choose another image.'); }
    finally {
      ...
      pending.current = false; if (active()) setBusy(false);
    }
  };
  const enabled = account.ready && !busy && (step === 0 ? !!name.trim() : !!photo);
  const next = async () => {
    if (!enabled || pending.current || !alive.current) return;
    pending.current = true; setBusy(true); setSubmitting(true); setError(''); Keyboard.dismiss();
    const attempt = ++generation.current;
    const active = () => alive.current && !closing.current && generation.current === attempt;
    ...
    if (active()) {
      ...
      if (!saved) setError('Profile changes could not be saved. Please try again.');
      else { session.current = null; if (step === 0) setStep(1); else onContinue(); }
    }
  };
