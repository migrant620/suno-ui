...
type SongPickerProps = {
    onClose: () => void;
    songs?: AudioTrack[];
    likedSongs?: AudioTrack[];
    publicSongs?: AudioTrack[];
} & ({
...
export function SongPicker({ onClose, onSelect, playlist, songs = [], likedSongs = [], publicSongs = [] }: SongPickerProps) {
    ...
    const previewSong = (song: AudioTrack, fromRow = false) => {
        try {
            if (selected?.id === song.id && playlist && fromRow) {
                preview.pause();
                setSelected(null);
            }
            else if (selected?.id === song.id) {
                if (previewStatus.playing)
                    preview.pause();
                else {
                    if (previewStatus.didJustFinish)
                        void preview.seekTo(0);
                    preview.play();
                }
            }
            else {
                setSelected(song);
                preview.replace(song.source);
                preview.play();
            }
            setPreviewError('');
        }
        catch {
            setPreviewError('Unable to preview this audio. You can retry or select another song.');
        }
    };
...
export function useTrackPlayback(track: AudioTrack | null) {
    ...
    useEffect(() => { if (track?.clip && status.playing && status.currentTime >= end) {
        player.pause();
        void player.seekTo(end).catch(() => setError('Unable to stop at the selected endpoint.'));
    } }, [track?.clip, status.playing, status.currentTime, end]);
    const seek = async (seconds: number) => {
        try {
            await player.seekTo(start + Math.max(0, Math.min(seconds, duration)));
            setError('');
        }
        catch {
            setError('Unable to seek in this audio. Try playing it again.');
        }
    };
    const play = async () => {
        try {
            if (status.playing)
                ...
                if (status.didJustFinish || current >= duration || status.currentTime < start)
            ...
            setError('This audio could not be played. Select another file or retry.');
        }
    };
    return { player, status, duration, current, error, setError, seek, play };
}
export function AudioChip({ track, playback, onRemove }: {
    track: AudioTrack;
    playback: ReturnType<typeof useTrackPlayback>;
    onRemove: () => void;
}) {
    const C = useSurfaceColors();
    const T = styles(C);
...
export function AudioAttachment({ track, onRemove, onExtendChange, mode, start, playback }: {
    ...
    mode: 'Cover' | 'Extend';
    start: number;
    onExtendChange: (mode: 'Cover' | 'Extend', start: number) => void;
}) {
    const C = useSurfaceColors();
    const T = styles(C);
    ...
    const setPoint = (x: number) => { const point = Math.max(0, Math.min(x / width, 1)) * duration; if (mode === 'Extend')
        onExtendChange(mode, point); void seek(point); };
    ...
    <View style={T.waveArea}>
      <View accessible accessibilityRole="adjustable" accessibilityLabel={mode === 'Extend' ? 'Extend start' : 'Audio position'} accessibilityValue={{ min: 0, max: duration, now: mode === 'Extend' ? start : current, text: clockTime(mode === 'Extend' ? start : current) }} accessibilityActions={[{ name: 'increment' }, { name: 'decrement' }]} onAccessibilityAction={event => setPoint(((mode === 'Extend' ? start : current) + (event.nativeEvent.actionName === 'increment' ? 1 : -1)) / (duration || 1) * width)} {...(Platform.OS === 'web' ? { tabIndex: 0, onKeyDown: (event: any) => { if (['ArrowRight', 'ArrowLeft'].includes(event.key)) {
            event.preventDefault();
            setPoint(((mode === 'Extend' ? start : current) + (event.key === 'ArrowRight' ? 1 : -1)) / (duration || 1) * width);
        } } } : {})} onLayout={event => setWidth(event.nativeEvent.layout.width)} onStartShouldSetResponder={() => true} onMoveShouldSetResponder={() => true} onResponderGrant={event => setPoint(event.nativeEvent.locationX)} onResponderMove={event => setPoint(event.nativeEvent.locationX)} style={T.wave}>
...
const styles = (C: ThemeColors) => StyleSheet.create({
    ...
    pickerTitle: { fontFamily: 'RobotoMedium', fontSize: 16, lineHeight: 20, letterSpacing: 0.15, textAlign: 'center', marginBottom: 14 },
    search: { marginHorizontal: 16, borderRadius: 8, height: 46, backgroundColor: C.surface === '#101012' ? C.control : '#1010120A', flexDirection: 'row', alignItems: 'center' },
