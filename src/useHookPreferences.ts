...
function read(raw: string | null): HookPreferences {
    ...
    if (!data || !['liked', 'hiddenCreators', 'skipped'].every(key => Array.isArray(data[key]) && data[key].every((id: unknown) => typeof id === 'string')) || !Array.isArray(data.reports))
        throw new Error('Invalid hook preferences');
    const created = data.created === undefined ? [] : data.created;
    if (!Array.isArray(created) || !created.every((item: any) => item && typeof item.id === 'string' && typeof item.templateId === 'string' && typeof item.songId === 'string' && typeof item.title === 'string' && typeof item.caption === 'string' && Number.isFinite(item.createdAt)))
        throw new Error('Invalid local Hooks');
    if (!data.reports.every((item: any) => item && typeof item.clipId === 'string' && typeof item.reason === 'string' && Number.isFinite(item.createdAt)))
        throw new Error('Invalid local reports');
    return { ...data, created };
}
export function useHookPreferences() {
    ...
    useEffect(() => { let alive = true; Storage.getItem(KEY).then(raw => { if (alive) {
        setData(read(raw));
        setReady(true);
    } }).catch(() => { if (alive)
        setError('Hook preferences could not load. Reload to retry.'); }); return () => { alive = false; }; }, []);
    const change = async (update: (current: HookPreferences) => HookPreferences) => {
        ...
        setError('');
        try {
            const saved = await Storage.updateItem(KEY, raw => JSON.stringify(update(read(raw))));
            setData(read(saved));
            return true;
        }
        catch {
            setError('Hook preferences could not be saved. Try again.');
            return false;
        }
        finally {
