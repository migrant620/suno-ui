...
export function MainNavigation({ tab, dark, avatarUri, onSelect, playback }: {
  tab: Tab; dark: boolean; avatarUri?: string; onSelect: (tab: Tab) => void;
  playback?: { track: AudioTrack; playing: boolean; onOpen: () => void; onToggle: () => void };
}) {
  ...
  return <View accessibilityLabel="Main navigation" testID="main-navigation" style={[styles.bar, { backgroundColor: surface }]}>
    {tabs.map(({ id, label }) => {
      ...
      if (id === 'create' && compact) return <View key={id} testID="navigation-compact-player" style={styles.compactSlot}>
      ...
      return <View key={id} style={[styles.slot, !!compact && { width: 68 }]}><Pressable accessibilityRole="button" accessibilityLabel={label}
        accessibilityState={{ selected }} aria-pressed={selected}
        onPress={() => onSelect(id)} style={({ pressed }) => [styles.target, id === 'create' && styles.createTarget, pressed && { opacity: 0.55 }]}>
