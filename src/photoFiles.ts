...
export async function readPhotoFile(id: string) {
  const uri = file(id); const info = await FileSystem.getInfoAsync(uri);
  if (!info.exists || info.isDirectory || !info.size) throw new Error('Your saved photo could not be opened.');
  return { uri, release: () => {} };
}
export async function removePhotoFile(id: string) {
  const uri = file(id);
  await FileSystem.deleteAsync(uri, { idempotent: true });
  if ((await FileSystem.getInfoAsync(uri)).exists) throw new Error('The photo could not be removed. Try again.');
}
export async function removeAllPhotoFiles() {
  const uri = directory();
  await FileSystem.deleteAsync(uri, { idempotent: true });
  if ((await FileSystem.getInfoAsync(uri)).exists) throw new Error('Your photo files could not be removed. Try again.');
}
...
export async function releasePhotoSource(uri: string) {
  ...
  if (!owned) return;
  await FileSystem.deleteAsync(uri, { idempotent: true });
  if ((await FileSystem.getInfoAsync(uri)).exists) throw new Error('The temporary photo could not be removed. Try again.');
}
