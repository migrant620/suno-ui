...
export function useLibrary() {
    ...
    const retryCleanup = () => queue(() => withAccountMedia(async () => {
        await cleanupMedia();
        const raw = await AsyncStorage.getItem(KEY);
        apply(raw ? readLibrary(raw) : emptyLibrary());
        setError('');
    ...
    useEffect(() => {
        ...
        const refresh = () => {
            void queue(async () => {
                try {
                    const raw = await AsyncStorage.getItem(KEY);
                    if (active) {
                        const next = raw ? readLibrary(raw) : emptyLibrary();
                        apply(next);
                        setReady(true);
                        if (next.removedMedia?.length)
                            void retryCleanup();
                    }
                }
                catch {
                    if (active)
                        setError('Your saved library could not be opened. Reload to retry.');
                }
            });
        };
        refresh();
        const listener = AppState.addEventListener('change', state => { if (state === 'active')
            refresh(); });
        const changed = (event: StorageEvent) => { if (event.key === KEY || event.key === null)
            refresh(); };
        if (Platform.OS === 'web')
            window.addEventListener('storage', changed);
        return () => { active = false; listener.remove(); if (Platform.OS === 'web')
    ...
    const remove = useCallback(async (id: string) => {
        ...
        try {
            ...
            setError('The playlist could not be deleted because its offline files could not be removed. Try again.');
            return false;
        }
    }, [ready]);
    const retainReceived = useCallback((current: LibraryData, songId: string): LibraryData => {
        if (current.songs?.some(song => song.id === songId) || current.receivedSongs?.some(song => song.id === songId))
            return current;
    ...
    const createSample = async (templateId: string, title: string, styles: string, lyrics: string, voiceId?: string, isActive: () => boolean = () => true, source?: CreationSource) => {
        if (!ready || !exampleTracks.some(track => track.id === templateId))
            throw new Error('Your library is not ready. Try again.');
        ...
        if (attachment)
            song.creation!.media = attachment;
            ...
            try {
                if (source?.media && attachment) {
                    ...
                    try {
                        ...
                        try {
                            const preview = await prepareMedia(copy.uri, attachment.kind);
                            preview.release();
                        }
                        finally {
                            copy.release();
                ...
                if (!isActive()) {
                    if (attachment)
                        await removeMediaFile(attachment.id, attachment.kind);
                    return null;
                }
                let raw: string;
                try {
                    raw = await AsyncStorage.updateItem(KEY, raw => {
                        if (!isActive())
                            throw new Error('Saving canceled.');
                        const current = raw ? readLibrary(raw) : emptyLibrary();
                    ...
                    if (!actual || !readLibrary(actual).songs?.some(item => item.id === song.id))
                        throw cause;
                    committed = true;
                    raw = actual;
                }
                ...
                apply(readLibrary(raw));
                setError('');
                return sampleTrack(song);
            }
            catch (cause) {
                if (!committed && attachment) {
                    const raw = await AsyncStorage.getItem(KEY);
                    if (!raw || !readLibrary(raw).songs?.some(item => item.id === song.id))
                        await removeMediaFile(attachment.id, attachment.kind);
                }
                throw cause;
            }
    ...
    const deleteSong = (id: string) => queue(() => withAccountMedia(async () => {
        ...
        try {
            ...
            apply(latest ? readLibrary(latest) : emptyLibrary());
            setError('');
        }
        catch {
            setError('The song was removed, but its saved media could not be removed. Retry media cleanup.');
        }
        return true;
