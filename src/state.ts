...
export type History = { past: string[]; value: string; future: string[] };
export type SavedText = { id: string; kind: EditorKind; name: string; value: string; createdAt: number };
export type Draft = {
  mode: 'Simple' | 'Advanced'; prompt: string; title: string; instrumental: boolean;
  lyrics: History; styles: History; saved: SavedText[]; voiceId?: string | null;
...
export type Action =
  | { type: 'field'; field: 'prompt' | 'title'; value: string }
  | { type: 'voice'; id: string | null }
  | { type: 'gender'; value: 'Male' | 'Female' | null }
  | { type: 'edit'; kind: EditorKind; value: string }
  | { type: 'undo' | 'redo' | 'reset'; kind: EditorKind }
  | { type: 'mode' | 'clear' | 'instrumental' }
  | { type: 'save'; item: SavedText }
  | { type: 'rename'; id: string; name: string }
  | { type: 'delete'; id: string }
  | { type: 'restore'; draft: Draft }
  | { type: 'remix'; track: AudioTrack };
