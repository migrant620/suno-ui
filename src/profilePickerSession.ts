...
export type ProfileForm = { name: string; handle: string; bio: string; avatarUri: string; coverUri: string; links: ProfileLinks };
export type ProfilePickerSession = {
  version: 1; id: string; epoch: string; accountId: string;
  screen: 'editor' | 'setup'; origin: 'profile' | 'settings'; target: 'avatar' | 'cover';
  original: ProfileForm; draft: ProfileForm; step: number; pending: boolean; error?: string;
};
...
function parse(raw: string | null): ProfilePickerSession | null {
  ...
  if (value.version !== 1 || !['id', 'epoch', 'accountId'].every(key => typeof value[key] === 'string' && value[key])
    ...
    || (value.error !== undefined && typeof value.error !== 'string')) throw new Error('Saved profile changes could not be opened.');
  return value;
}
...
export async function persistProfilePicker(session: ProfilePickerSession, active: () => boolean, create = false) {
  ...
  await Storage.updateItem(PROFILE_PICKER_KEY, raw => {
    if (!active() || session.epoch !== accountEpoch()) throw new Error('Profile selection cancelled');
    const previous = parse(raw);
    if (!previous && !create) throw new Error('Profile changes were already closed.');
    if (previous && previous.id !== session.id) throw new Error('Other profile changes are still open.');
...
export async function clearProfilePicker(id?: string) {
  if (Platform.OS !== 'android') return;
  await Storage.updateItem(PROFILE_PICKER_KEY, raw => {
    if (!id) return 'null'; // Explicit discard from the recovery error screen.
    const value = parse(raw);
    return value?.id === id ? 'null' : raw || 'null';
  });
  if (id) pendingResults.delete(id); else pendingResults.clear();
}
...
async function recoverPendingPicker(accountId: string, active: () => boolean): Promise<ProfilePickerSession | null> {
  ...
  if (session.pending) {
    let error = '';
    try {
      // The native result is consumed once. Retain it through a storage retry or
      // React remount until its recovered image is durably recorded.
      if (!pendingResults.has(session.id)) pendingResults.set(session.id, ImagePicker.getPendingResultAsync());
      const result = await pendingResults.get(session.id)!;
      if (result && 'code' in result) error = 'The photo could not be recovered. Your other changes are still here. Choose the photo again.';
    ...
    } catch { error = 'The photo could not be recovered. Your other changes are still here. Choose the photo again.'; }
