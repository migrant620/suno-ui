...
export function AddToSongMenu({ anchor, onClose, onVoice, onImage, onVideo, onAdvanced }: {
  anchor: React.RefObject<View | null>; onClose: () => void; onVoice: () => void; onImage: () => void; onVideo: () => void; onAdvanced: () => void;
}) {
  ...
  useEffect(() => {
    ...
    anchor.current?.measureInWindow((x, y, _w, h) => {
      if (active) setPosition({ left: Math.max(8, Math.min(x, width - 188)), top: Math.max(insets.top, Math.min(y + h, height - insets.bottom - 216)) });
    });
    return () => { active = false; };
  }, [anchor, width, height, insets.top, insets.bottom]);
  const items: { title: string; icon: IconName; size?: number; action: () => void }[] = [
