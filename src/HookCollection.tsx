...
export function HookCollection({ item, library, player, onBack, onCreate, onSongOptions }: {
    ...
    const toggle = async () => {
        ...
        try {
            ...
            if (!ok)
                setError('The collection could not be updated. Try again.');
        }
        finally {
    ...
    return <View ref={root} style={{ flex: 1 }}><PlaylistDetails item={item} liked={false} shared tracks={item.songIds.map(id => exampleTracks.find(track => track.id === id)).filter((track): track is AudioTrack => !!track)} player={player} onBack={onBack} backLabel="Back to Hooks" onCreate={onCreate} onAdd={() => void toggle()} onMenu={() => void toggle()} onSongOptions={onSongOptions} curated={{ saved, busy, error, onToggle: () => void toggle() }}/></View>;
}
