...
function readProfile(raw: string | null): LocalProfile {
    ...
    if (typeof value.name !== 'string' || !value.name.trim() || typeof value.bio !== 'string' || typeof value.reduceMotion !== 'boolean')
        throw new Error('Invalid profile');
    if (value.followedCreatorIds !== undefined && (!Array.isArray(value.followedCreatorIds) || !value.followedCreatorIds.every((id: unknown) => typeof id === 'string')))
        throw new Error('Invalid following list');
    if (value.likedVoiceIds !== undefined && (!Array.isArray(value.likedVoiceIds) || !value.likedVoiceIds.every((id: unknown) => typeof id === 'string')))
        throw new Error('Invalid liked Voices');
    for (const key of ['handle', 'avatarUri', 'coverUri', 'shareId'])
        if (value[key] !== undefined && typeof value[key] !== 'string')
            throw new Error('Invalid profile field');
    if (value.links !== undefined && (!value.links || typeof value.links !== 'object' || Array.isArray(value.links) || !Object.entries(value.links).every(([key, link]) => socialFields.some(field => field.key === key) && typeof link === 'string')))
        throw new Error('Invalid profile links');
    if (value.hiddenRecommendations !== undefined && typeof value.hiddenRecommendations !== 'boolean')
        throw new Error('Invalid recommendations state');
    if (value.dismissedCreatorIds !== undefined && (!Array.isArray(value.dismissedCreatorIds) || !value.dismissedCreatorIds.every((id: unknown) => typeof id === 'string')))
        throw new Error('Invalid recommendation exclusions');
    if (value.completionStep !== undefined && (!Number.isInteger(value.completionStep) || value.completionStep < 0 || value.completionStep > 4))
        throw new Error('Invalid profile completion');
    return value;
}
...
export function useLocalProfile() {
            ...
            try {
                ...
                if (!raw || !current.shareId) {
                    current = readProfile(await AsyncStorage.updateItem(key, latest => {
                        if (!active)
                            throw new Error('Profile closed');
    ...
    const update = async (patch: ProfilePatch, isActive: () => boolean = () => true, pickerSessionId?: string): Promise<boolean> => {
        ...
        try {
            ...
            if (mounted.current && isActive() && attempt === revision.current)
