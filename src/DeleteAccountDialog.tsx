...
export function DeleteAccountDialog({ onClose }: { onClose: () => void }) {
  ...
  const [confirmation, setConfirmation] = useState(''); const [focused, setFocused] = useState(false); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); const changing = useRef(false);
  const cancel = () => { if (!changing.current) { Keyboard.dismiss(); onClose(); } };
  const remove = async () => {
    if (changing.current) return;
    if (confirmation !== 'confirm_delete') { setError("Enter exactly 'confirm_delete' to delete this local account."); return; }
    changing.current = true; setBusy(true); setError(''); Keyboard.dismiss();
    try { await session.deleteAccount(); }
    catch (e) { setError(e instanceof Error && e.message.includes('HTTPS') ? e.message : 'Deletion could not start. Your saved data has not been removed. Try again.'); changing.current = false; setBusy(false); }
  };
