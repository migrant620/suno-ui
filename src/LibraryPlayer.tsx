...
export function LibraryPlayer({ player, library, community, author, onClose, onReuse }: {
  player: ReturnType<typeof useLibraryPlayback>; library: ReturnType<typeof useLibrary>;
  community: ReturnType<typeof useCommunity>; author: string;
  onClose: () => void; onReuse: (track: AudioTrack, withAudio: boolean) => void;
}) {
  ...
  const copyStyles = async () => {
    try { await Clipboard.setStringAsync(track.styles); setNotice('Styles copied'); }
    catch { setNotice('Styles could not be copied. Try again.'); }
  };
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={P.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={P.timeline}>
            <View accessible accessibilityRole="adjustable" accessibilityLabel="Song playback position" accessibilityValue={{ min: 0, max: player.duration, now: player.current, text: clockTime(player.current) }} accessibilityActions={[{ name: 'increment' }, { name: 'decrement' }]}
              onAccessibilityAction={e => void player.seek(player.current + (e.nativeEvent.actionName === 'increment' ? 5 : -5))}
              {...(Platform.OS === 'web' ? { tabIndex: 0, onKeyDown: (e: any) => { if (['ArrowRight', 'ArrowLeft', 'Home', 'End'].includes(e.key)) { e.preventDefault(); void player.seek(e.key === 'Home' ? 0 : e.key === 'End' ? player.duration : player.current + (e.key === 'ArrowRight' ? 5 : -5)); } } } : {})}
