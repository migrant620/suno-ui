...
function read(raw: string | null): RecentSearch[] {
    ...
    if (value?.version !== 1 || !Array.isArray(value.entries) || !value.entries.every((item: RecentSearch) => item && ['creator', 'song', 'playlist'].includes(item.kind) && typeof item.id === 'string' && item.id.length > 0))
        throw new Error('Invalid search history');
    const seen = new Set<string>();
    return value.entries.filter((item: RecentSearch) => { const key = recentKey(item); if (seen.has(key))
...
export function useSearchHistory() {
    ...
    async function drain() {
        if (!mounted.current || !loaded.current || running.current)
            return;
        ...
        try {
            ...
            if (mounted.current)
                setError('Search history could not be saved. Retry to keep your changes.');
        }
        finally {
    ...
    async function refresh() {
        if (!mounted.current)
            return;
        if (loading.current || running.current) {
        ...
        try {
            ...
            if (mounted.current) {
                ...
                setReady(false);
                setError('Search history could not be loaded. Retry or clear this local history.');
            }
        }
        finally {
    ...
    useEffect(() => {
        ...
        void refresh();
        const subscription = AppState.addEventListener('change', value => { if (value === 'active')
            void refresh(); });
        const storageChanged = (event: StorageEvent) => { if (event.key === SEARCH_HISTORY_KEY)
            void refresh(); };
        if (Platform.OS === 'web')
            window.addEventListener('storage', storageChanged);
        return () => { mounted.current = false; subscription.remove(); if (Platform.OS === 'web')
    ...
    return {
        ...
        clear: () => { if (!loaded.current) {
            pending.current = [];
            loaded.current = true;
            setReady(true);
        } enqueue({ type: 'clear' }); },
        retry: () => { if (loaded.current)
            void drain();
        else
            void refresh(); },
        refresh,
