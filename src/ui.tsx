...
export function Icon({ name, size = 24, color: suppliedColor }: {
    name: IconName;
    size?: number;
    color?: string;
}) {
    const colors = useSurfaceColors();
    const color = suppliedColor || colors.ink;
    const Vector = name === 'undo' ? Undo2 : name === 'redo' ? Redo2 : name === 'bookshelf' ? Library : null;
    if (Vector)
        return <View accessible={false} aria-hidden importantForAccessibility="no-hide-descendants"><Vector size={size} color={color} strokeWidth={2.5}/></View>;
    if (name === 'delete')
        return <Ionicons name="trash" size={size} color={color} accessible={false} aria-hidden importantForAccessibility="no-hide-descendants"/>;
    if (name === 'thumb-up')
        return <MaterialIcons name="thumb-up-alt" size={size} color={color} accessible={false} aria-hidden importantForAccessibility="no-hide-descendants"/>;
...
export function IconButton({ name, label, onPress, disabled, color, style, size = 24, circle }: {
    ...
    return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: !!disabled }} disabled={disabled} onPress={onPress} style={({ pressed }) => [S.iconButton, style, pressed && { opacity: 0.55 }]}>
...
export function Sheet({ children, onClose, compact = false, onShow, backgroundColor, backdrop, keyboardAvoiding = false, handleColor, handleHeight, handlePaddingTop }: {
    children: React.ReactNode;
    onClose: () => void;
    compact?: boolean;
    onShow?: () => void;
...
export const S = StyleSheet.create({
    ...
    pillContent: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingLeft: 11, paddingRight: 19, height: 40 },
