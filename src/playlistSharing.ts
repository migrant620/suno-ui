...
export function decodeSharedPlaylist(raw: string): Playlist | null {
    ...
    try {
        ...
        if (!Array.isArray(incomingSongs) || incomingSongs.length > 100 || !incomingSongs.every(isSampleSong))
            return null;
        const songs = incomingSongs.map(song => savedSample(sampleTrack(song))!);
        if (!Array.isArray(songs) || songs.length > 100 || !songs.every(isSampleSong))
            return null;
        const allowed = new Set([...exampleTracks.map(track => track.id), ...songs.map(song => song.id)]);
        if (value?.version !== 1 || typeof value.name !== 'string' || !value.name.trim() || value.name.length > 2000 || typeof value.description !== 'string' || value.description.length > 200 || !Array.isArray(value.songIds) || value.songIds.length > 100 || !value.songIds.every((id: unknown) => typeof id === 'string' && allowed.has(id)))
            return null;
