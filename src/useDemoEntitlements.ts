...
export function useDemoEntitlements() {
    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem(KEY).then(raw => {
            ...
            if (!saved || !['Free', 'Pro', 'Premier'].includes(saved.plan) || !['Monthly', 'Annual'].includes(saved.billing) || !Number.isSafeInteger(saved.downloads) || saved.downloads < 0 || !['v6', 'v6-wild', 'v6-mini'].includes(saved.model) || !Array.isArray(saved.transactions) || !saved.transactions.every((id: unknown) => typeof id === 'string'))
            ...
            setError('Your demo plan could not be loaded. Reload to retry.'); });
        return () => { active.current = false; };
    }, []);
    const commit = (transform: (value: Entitlements) => Entitlements) => {
        if (!ready)
            return Promise.reject(new Error('Your demo plan is not ready.'));
    ...
    const purchase = (order: DemoOrder, transaction: string) => commit(value => {
        if (value.transactions.includes(transaction))
            return value;
    ...
    const selectModel = (model: MusicModel) => commit(value => {
        if (value.plan === 'Free' && model !== 'v6-mini')
            throw new Error('This model requires a demo Pro or Premier plan.');
        return { ...value, model };
    });
