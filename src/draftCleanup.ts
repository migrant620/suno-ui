...
export function parseAttachmentCleanup(raw: string | null): AttachmentCleanup[] {
    ...
    if (value?.version !== 1 || !Array.isArray(value.items) || !value.items.every((item: AttachmentCleanup) => item && typeof item.id === 'string' && /^[a-zA-Z0-9][a-zA-Z0-9:_-]{0,199}$/.test(item.id) && (item.type === 'audio' || (item.type === 'media' && ['images', 'videos'].includes(item.kind)))))
        throw new Error('Previous attachment cleanup could not be read. Try again.');
    return value.items;
}
...
export async function flushAttachmentCleanupLocked() {
    ...
    for (const item of items) {
        if (item.type === 'media' ? activeMedia?.id === item.id : activeAudio?.track?.id === item.id)
            continue;
        try {
            if (item.type === 'media')
                await removeMediaFile(item.id, item.kind);
            else
                await removeDraftAudioFile(item.id);
        }
        catch {
            remaining.push(item);
        }
    ...
    if (raw === null)
        await Storage.removeItem(DRAFT_CLEANUP_KEY);
    else
        await Storage.setItem(DRAFT_CLEANUP_KEY, raw);
