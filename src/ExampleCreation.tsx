...
export function ExampleCreation({ title, ready, voiceName, mediaName, onClose, onCreate }: {
    title: string;
    ready: boolean;
    voiceName?: string;
    mediaName?: string;
    onClose: () => void;
    onCreate: (template: string, isActive: () => boolean) => Promise<void>;
}) {
    ...
    const save = async () => {
        if (!ready || pending.current || !alive.current)
            return;
        pending.current = true;
        setSaving(true);
        setError('');
        try {
            await onCreate(selected, () => alive.current);
        }
        catch {
            if (alive.current)
                setError('Your example song could not be saved. Your details are still here. Try again.');
        }
        finally {
