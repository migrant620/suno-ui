...
function read(raw: string): Community {
    ...
    if (!data || !Array.isArray(data.comments) || !data.drafts || typeof data.drafts !== 'object' || Array.isArray(data.drafts))
    ...
    if (!Object.values(data.drafts).every(value => typeof value === 'string'))
        throw new Error('Invalid draft');
    if (data.replyTargets !== undefined && (!data.replyTargets || typeof data.replyTargets !== 'object' || Array.isArray(data.replyTargets) || !Object.values(data.replyTargets).every(value => typeof value === 'string')))
...
export function useCommunity() {
    ...
    const edit = (kind: 'drafts' | 'replyTargets', trackId: string, value: string) => {
        ...
        void enqueue(async () => {
            try {
                ...
                base.current = read(raw);
                if (overlays.current.get(trackId)?.revision === item.revision)
                    overlays.current.delete(trackId);
                if (active.current) {
                    setError('');
                    show();
                }
    ...
    const add = async (trackId: string, author: string, text: string, at: number, parentId?: string, isActive: () => boolean = () => true): Promise<boolean> => {
        if (!ready || sending.current || !text.trim())
            return false;
        ...
        try {
            return await enqueue(async () => {
                ...
                try {
                    saved = read(await AsyncStorage.updateItem(KEY, raw => {
                        ...
                        const current = raw ? read(raw) : initial();
                        if (parentId && !current.comments.some(item => item.id === parentId && item.trackId === trackId))
                    ...
                    if (!recovered?.comments.some(item => item.id === comment.id))
                        throw cause;
                    saved = recovered;
                }
                base.current = saved;
                if (draftEdits.current.get(trackId)?.revision === draftRevision)
                    draftEdits.current.delete(trackId);
                if (replyEdits.current.get(trackId)?.revision === replyRevision)
                    replyEdits.current.delete(trackId);
            ...
            if (active.current && isActive())
                setError('Your comment could not be saved. Try posting again.');
            return false;
        }
        finally {
    ...
    const remove = (id: string, isActive: () => boolean = () => true) => updateComments(current => {
        if (!current.comments.some(comment => comment.id === id && comment.own))
            return current;
