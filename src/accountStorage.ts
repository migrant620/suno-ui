...
export async function readAccountState(): Promise<AccountState> {
    ...
    if (!raw)
        return { phase: 'active', epoch: 'legacy' };
    const value = JSON.parse(raw);
    if (!['active', 'closing', 'closed'].includes(value?.phase) || typeof value.epoch !== 'string' || !value.epoch)
        throw new Error('Your local account state could not be read.');
    return value;
}
...
async function assertActive(expected: string) {
    ...
    if (current.phase !== 'active' || current.epoch !== expected)
        throw new Error('This local account changed. Reopen the demo before saving.');
}
...
const accountStorage = {
    ...
    updateItem: (key: string, update: (raw: string | null) => string, clearMatching?: {
        key: string;
        id: string;
    }) => write(key, async () => {
        ...
        if (clearMatching) {
            ...
            if (!companion || JSON.parse(companion)?.id !== clearMatching.id)
                throw new Error('The pending edit changed.');
...
export async function finishAccountDeletion() {
    ...
    await withAccountMutex('account-media', () => withAccountMutex('account-storage', async () => {
        ...
        if (state.phase !== 'closing')
            throw new Error('Account deletion has not been confirmed.');
        await deleteAccountFiles();
        await Storage.multiRemove(accountKeys);
        for (const key of accountKeys)
            if (await Storage.getItem(key) !== null)
                throw new Error('Some saved data could not be removed.');
...
export async function openLocalAccount() {
    return withAccountMutex('account-storage', async () => {
        const state = await readAccountState();
        if (state.phase === 'closing')
            throw new Error('Finish removing the previous local account first.');
        ...
        if (state.phase === 'closed')
            await Storage.setItem(ACCOUNT_CONTROL, JSON.stringify(next));
        bindAccount(next);
    });
}
