...
export function VoiceTrim({ track, peaks, duration, name, placeholder, range, onName, onRange, onStartOver, onNext, bottom }: {
    ...
    useEffect(() => {
        if (playback.status.playing && playback.current >= range.end && !stopping.current) {
            ...
            void playback.seek(range.end).finally(() => { stopping.current = false; });
        }
    ...
    const play = async () => {
        ...
        if (playback.current < range.start || playback.current >= range.end)
            await playback.seek(range.start);
        await playback.player.play();
    };
    const change = (kind: 'start' | 'end' | 'move', delta: number, initial = range) => {
        playback.player.pause();
        if (kind === 'start')
            onRange({ start: Math.max(0, Math.min(initial.end - 15, initial.start + delta)), end: initial.end });
        else if (kind === 'end')
            onRange({ start: initial.start, end: Math.min(duration, Math.max(initial.start + 15, initial.end + delta)) });
        else {
            const length = initial.end - initial.start;
            const start = Math.max(0, Math.min(duration - length, initial.start + delta));
    ...
    const slider = (kind: 'start' | 'end' | 'move') => ({
        ...
        ...(Platform.OS === 'web' ? { tabIndex: 0 as const, onKeyDown: (event: any) => { if (['ArrowRight', 'ArrowLeft'].includes(event.key)) {
                event.preventDefault();
                change(kind, event.key === 'ArrowRight' ? 1 : -1);
            } } } : {}),
        onStartShouldSetResponder: () => true,
        onMoveShouldSetResponder: () => true,
        onResponderGrant: (event: any) => { playback.player.pause(); drag.current = { kind, x: event.nativeEvent.pageX, initial: range }; },
        onResponderMove: (event: any) => { if (drag.current)
            change(kind, (event.nativeEvent.pageX - drag.current.x) / width * duration, drag.current.initial); },
    ...
    const bars = Array.from({ length: count }, (_, index) => Math.max(0, ...peaks.slice(Math.floor(index / count * peaks.length), Math.max(Math.floor(index / count * peaks.length) + 1, Math.ceil((index + 1) / count * peaks.length)))));
