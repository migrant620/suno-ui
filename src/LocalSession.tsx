...
type Session = { leave: () => Promise<void>; deleteAccount: () => Promise<void> };
...
export function LocalSession({ children }: { children: React.ReactNode }) {
  ...
  const load = async (initial = false) => {
    ...
    try {
      ...
      if (saved !== null && !['active', 'signed-out'].includes(saved)) throw new Error('Invalid session');
      if (initial) bindAccount(state);
      if (state.phase === 'active' && saved !== 'signed-out' && state.epoch === accountEpoch()) await accountStorage.recoverDraft();
      ...
      setError('');
    } catch { if (request === reading.current) { setPhase('loading'); setError('Your local session could not be loaded. Try again.'); } }
  };
  useEffect(() => {
    void load(true);
    const changed = (event: StorageEvent) => { if ([ACCOUNT_CONTROL, SESSION_KEY].includes(event.key || '')) void load(); };
    ...
    return () => { reading.current += 1; subscription.remove(); if (Platform.OS === 'web') window.removeEventListener('storage', changed); };
  }, []);
  const finish = async () => {
    if (changing.current) return;
    changing.current = true; setBusy(true); setError('');
    try { await finishAccountDeletion(); setPhase('closed'); }
    catch { setError('Some local data could not be removed. Retry to finish deleting this local account.'); }
    finally { changing.current = false; setBusy(false); }
  };
  // Starting only after this commit lets child media and save effects unmount first.
  useEffect(() => { if (phase === 'closing') void finish(); }, [phase]);
  const leave = async () => {
    if (changing.current) return;
    changing.current = true;
    try { await accountStorage.setItem(SESSION_KEY, 'signed-out'); setPhase('signed-out'); }
    finally { changing.current = false; }
  };
  const remove = async () => {
    if (changing.current) return;
    changing.current = true;
    try { await beginAccountDeletion(); setError(''); setPhase('closing'); }
    finally { changing.current = false; }
  };
  const open = async () => {
    if (changing.current) return;
    changing.current = true; setBusy(true);
    try { await openLocalAccount(); setError(''); setPhase('active'); }
    catch { setError('The local session could not be saved. Try again.'); }
    finally { changing.current = false; setBusy(false); }
  };
  if (phase === 'active') return <Context.Provider value={{ leave, deleteAccount: remove }}>{children}</Context.Provider>;
