...
export function Comments({ track, currentTime, author, community, onSeek, onClose, timestamps = true }: {
    timestamps?: boolean;
    track: AudioTrack;
    currentTime: number;
    author: string;
    ...
    const confirmDelete = async () => {
        ...
        if (!deleting || !attempt?.active || attempt.pending)
            return;
        ...
        try {
            ...
            if (mounted.current && deletion.current === attempt)
                setDeleteBusy(false);
        }
    };
    ...
    const copy = async (comment: LocalComment) => { try {
        await Clipboard.setStringAsync(comment.text);
        setNotice('Comment copied');
    }
    catch {
        setNotice('Comment could not be copied. Try again.');
    } setOptions(null); };
