...
export function PlaylistEditor({ item, onClose, onSave }: {
    item: Playlist;
    onClose: () => void;
    onSave: (item: Playlist) => void;
}) {
    ...
    const chooseCover = async () => {
        try {
            ...
            if (!result.canceled) {
                ...
                if (!asset.base64)
                    throw new Error('No persistent image content');
                setCoverUri(`data:${asset.mimeType || 'image/jpeg'};base64,${asset.base64}`);
                setError('');
            }
        }
        catch {
            setError('The cover could not be opened. Please try another image.');
        }
    };
