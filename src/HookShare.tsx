...
export function HookShare({ clip, creator, onClose }: {
    clip: ExampleHook;
    creator: string;
    onClose: () => void;
}) {
    ...
    const send = async (copy: boolean) => {
        ...
        setBusy(true);
        try {
            const url = Linking.createURL('', { queryParams: { hook: encodeHook(clip, creator) } });
            if (copy || (Platform.OS === 'web' && !navigator.share)) {
                await Clipboard.setStringAsync(url);
                setNotice('Hook link copied');
            }
            else
                await Share.share({ title: clip.track.title, message: `${clip.track.title} · Local example Hook\n${url}` });
        }
        catch (error) {
            if (!(error instanceof Error && error.name === 'AbortError'))
                setNotice('Sharing could not open. Try Copy Link.');
        }
        finally {
            setBusy(false);
        }
