...
export function usePlaylistDownload(playlistId: string, tracks: AudioTrack[]) {
    ...
    useEffect(() => {
        ...
        scope.current += 1;
        let active = true;
        hasOfflinePlaylist(playlistId, tracks).then(saved => { if (active)
            setStatus(saved ? 'downloaded' : 'idle'); }).catch(() => { if (active)
            setError('Offline storage could not be checked.'); });
        return () => { active = false; mounted.current = false; task.current?.abort(); };
    }, [playlistId, membership]);
    const download = async () => {
        if (task.current || !tracks.length)
            return;
        ...
        try {
            ...
            if (mounted.current && scope.current === version)
            ...
            if (mounted.current && scope.current === version) {
                ...
                if (!controller.signal.aborted)
                    setError('The playlist could not be downloaded. Check your connection and retry.');
            }
        }
        finally {
    ...
    const remove = async () => {
        try {
            ...
            setError('Offline audio could not be removed. Try again.');
        }
    };
