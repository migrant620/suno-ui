...
export async function reuseDraft(track: AudioTrack, draft: Draft, expected: DraftRecords, isActive: () => boolean) {
  return withOfflineLock('remix-draft', async () => {
    ...
    for (const key of [textKey, audioKey, mediaKey] as const) {
      if (await Storage.getItem(key) !== expected[key]) throw new Error('Your draft changed. Reopen Remix before trying again.');
    }
    ...
    let oldAudio: { track: AudioTrack } | null = null;
    try { oldMedia = expected[mediaKey] ? JSON.parse(expected[mediaKey]!) : null; } catch { /* Unknown files stay intact. */ }
    try { oldAudio = expected[audioKey] ? JSON.parse(expected[audioKey]!) : null; } catch { /* Explicit reuse can repair invalid metadata. */ }
    ...
    try {
      ...
      if (attached) {
        ...
        try {
          check();
          const preview = await prepareMedia(borrowed.uri, attached.kind); preview.release();
          await saveMediaFile(nextMedia.id, borrowed.uri, attached.kind);
          const saved = await readMediaFile(nextMedia.id, attached.kind);
          try { const decoded = await prepareMedia(saved.uri, attached.kind); decoded.release(); } finally { saved.release(); }
      ...
      if (!example(track) && oldAudio?.track?.id !== track.id) {
        ...
        const saved = await readDraftAudioFile(track.id); saved.release();
      }
      check();
      ...
      if (oldMedia?.id && oldMedia.id !== nextMedia?.id) cleanup.push({ type: 'media', id: oldMedia.id, kind: oldMedia.kind });
      if (oldAudio?.track?.id && oldAudio.track.id !== track.id && !example(oldAudio.track)) cleanup.push({ type: 'audio', id: oldAudio.track.id });
      await Storage.commitDraft(records, expected, isActive, { before: cleanupBefore, after: serializeAttachmentCleanup(cleanup) });
      committed = true;
      // Old files remain valid until all three metadata records commit together.
      // Cleanup failure must not report that the committed draft was discarded.
      try {
        await flushAttachmentCleanupLocked();
      } catch { /* The committed cleanup queue remains available for retry. */ }
      return { track, edit, media: nextMedia, records };
    } catch (error) {
      if (!committed) {
        // This read first recovers a pending journal. If recovery still fails,
        // retain both sets of files rather than guessing which record survived.
        const actualMedia = await Storage.getItem(mediaKey);
        const actualAudio = await Storage.getItem(audioKey);
        if (nextMedia && actualMedia !== JSON.stringify(nextMedia)) await removeMediaFile(nextMedia.id, nextMedia.kind);
        if (copiedAudio && actualAudio !== audioRecord) await removeDraftAudioFile(track.id);
      }
      throw error;
    }
  });
