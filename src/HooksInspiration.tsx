...
export function HooksInspiration({ onSelect, scrollOffsets }: { onSelect: (kind: 'photo' | 'bedtime' | 'love') => void; scrollOffsets: React.MutableRefObject<Record<string, number>> }) {
  ...
  useEffect(() => {
    ...
    motion.start(); return () => { clearInterval(timer); motion.stop(); };
  }, [rotation]);
...
const S = StyleSheet.create({
  section: { flexShrink: 0 }, heading: { fontSize: 18, lineHeight: 24, color: '#F7F4EF', marginHorizontal: 16, marginTop: 11, marginBottom: 7, fontFamily: 'RobotoMedium' },  // hooks02: heading line at 76
