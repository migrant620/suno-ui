...
export function CommentOptionsMenu({ anchor, onClose, onReport }: {
    ...
    useEffect(() => {
        ...
        anchor.current?.measureInWindow((x, y, w, h) => {
            if (active)
                setPosition({ left: Math.max(8, Math.min(x + w - 12 - 112, width - 120)), top: Math.max(insets.top, Math.min(y + h / 2 + 11, height - insets.bottom - 72)) });
        });
        return () => { active = false; };
...
export function CommentReport({ comment, onClose, onSaved }: {
    ...
    const save = async () => {
        ...
        try {
            await Storage.updateItem(COMMENT_REPORTS_KEY, raw => {
                ...
                if (data?.version !== 1 || !Array.isArray(data.reports) || !data.reports.every((item: any) => item && typeof item.commentId === 'string' && typeof item.trackId === 'string' && Number.isFinite(item.createdAt)))
                    throw new Error('Invalid reports');
                if (!data.reports.some((item: any) => item.commentId === comment.id && item.trackId === comment.trackId))
                    data.reports.push({ commentId: comment.id, trackId: comment.trackId, createdAt: Date.now() });
