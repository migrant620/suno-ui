...
export function HookRemix({ track, library, onClose, onAdvanced, onCreated }: { track: AudioTrack; library: ReturnType<typeof useLibrary>; onClose: () => void; onAdvanced: (track: AudioTrack, isActive: () => boolean) => Promise<boolean>; onCreated: (track: AudioTrack) => void }) {
  const [prompt, setPrompt] = useState(''); const [styles, setStyles] = useState(track.styles.split(',').map(value => value.trim()).filter(Boolean));
  const [creating, setCreating] = useState(false); const input = useRef<TextInput>(null);
  const leavingInput = useRef(false); const keyboardShown = useRef(false);
  const alive = useRef(true); const pendingAdvanced = useRef(false);
  const [advancedBusy, setAdvancedBusy] = useState(false); const [advancedError, setAdvancedError] = useState('');
  useEffect(() => { alive.current = true; return () => { alive.current = false; }; }, []);
  useEffect(() => {
    if (Platform.OS !== 'android') return;
    const shown = Keyboard.addListener('keyboardDidShow', () => { keyboardShown.current = true; });
    const hidden = Keyboard.addListener('keyboardDidHide', () => {
      const wasShown = keyboardShown.current; keyboardShown.current = false;
      if (wasShown && !leavingInput.current && AppState.currentState === 'active') onClose();
    });
    return () => { shown.remove(); hidden.remove(); };
  }, [onClose]);
  const leaveInput = () => { leavingInput.current = true; Keyboard.dismiss(); };
  const startCreation = () => { if (pendingAdvanced.current) return; leaveInput(); setCreating(true); };
  const focusInput = () => {
    const target = input.current;
    if (Platform.OS === 'android' && NativeModules.SunoKeyboard) {
      const tag = findNodeHandle(target);
      if (tag !== null) NativeModules.SunoKeyboard.focusInWindow(tag);
    } else target?.focus();
  };
  ...
  const openAdvanced = async () => {
    if (pendingAdvanced.current) return;
    pendingAdvanced.current = true; setAdvancedBusy(true); setAdvancedError(''); leaveInput();
    try {
      const saved = await onAdvanced({ ...track, styles: styles.join(', '), creation }, () => alive.current);
      if (!saved && alive.current) { leavingInput.current = false; setAdvancedError('Remix could not be opened. Try Advanced again.'); }
    } catch {
      if (alive.current) { leavingInput.current = false; setAdvancedError('Remix could not be opened. Try Advanced again.'); }
    } finally {
      pendingAdvanced.current = false;
      if (alive.current) setAdvancedBusy(false);
    }
  };
  return <Sheet compact keyboardAvoiding onClose={() => { alive.current = false; onClose(); }} onShow={focusInput} backgroundColor="#1C1C1F" backdrop={<CoverBackdrop source={track.cover} />} handleColor="#C2C2C1" handleHeight={48}>
    ...
    {creating && <ExampleCreation title={prompt.trim().slice(0, 70) || `${track.title} Remix`} ready={library.ready} onClose={() => { leavingInput.current = false; setCreating(false); }} onCreate={async (template, isActive) => {
      ...
      if (result && isActive()) { setCreating(false); onCreated(result); }
    }} />}
  </Sheet>;
}
