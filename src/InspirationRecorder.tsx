...
export function InspirationRecorder({ onClose, onUse }: { onClose: () => void; onUse: (track: AudioTrack) => void }) {
  const recorder = useRecorder();
  const insets = useSafeAreaInsets(); const [height, setHeight] = useState(0);
  const root = useRef<View>(null);
  const held = useRef(false);
  const starting = useRef<Promise<void> | null>(null);
  const current = useRef(recorder); current.current = recorder;
  ...
  const begin = () => {
    held.current = true;
    if (starting.current || current.current.recording || current.current.file) return;
    starting.current = (async () => {
      await current.current.start();
      starting.current = null;
      if (!held.current) await current.current.stop();
    })();
  };
    ...
    <View style={S.outer}><SafeAreaView ref={root} edges={['top', 'bottom']} style={S.page} onLayout={event => setHeight(event.nativeEvent.layout.height)}>
      ...
      <View style={S.footer}>
        ...
        {track ? <View style={S.review}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Use bedtime recording" style={S.use} onPress={() => { playback.player.pause(); if (recorder.take()) onUse(track); }}><Label style={S.useText}>Continue to Create</Label></Pressable>
        </View> : <Pressable accessibilityRole="button" accessibilityLabel="Hold to record" accessibilityHint="Hold for at least six seconds. Release to review your local recording." onPressIn={begin} onPressOut={end}
          accessibilityActions={[{ name: 'start', label: 'Start recording' }, { name: 'stop', label: 'Stop recording' }]} onAccessibilityAction={event => event.nativeEvent.actionName === 'start' ? begin() : end()}
          testID="inspiration-record-target" style={S.record}>{recorder.busy ? <ActivityIndicator color="#C32906" /> : <View style={[S.dot, recorder.recording && S.recording]} />}</Pressable>}
      </View>
    </SafeAreaView></View>
  </Modal>;
}
