...
function parse(raw: string): PhotoDraft {
    ...
    try {
        ...
        throw new Error('Your photo draft could not be opened. Retry or discard it to choose another photo.');
    }
    if (value?.version !== 1 || !value.draft || !['id', 'name', 'place', 'moment', 'style'].every(k => typeof value.draft[k] === 'string') || !/^photo-[a-z0-9-]+$/.test(value.draft.id))
        throw new Error('Your photo draft could not be opened.');
    return value.draft;
}
export function usePhotoDraft() {
    ...
    const finishSource = async (id: string) => {
        ...
        if (source) {
            await releasePhotoSource(source);
    ...
    const run = async (job: () => Promise<void>, retryable = true) => {
        ...
        try {
            const task = withOfflineLock(PHOTO_DRAFT_KEY, job);
            latestJob.current = task;
            await task;
            if (retryable)
                retryJob.current = null;
            if (active.current && !retryJob.current)
                setError('');
            return true;
        }
        catch (cause) {
            ...
            if (active.current && (retryable || !retryJob.current))
                setError(cause instanceof Error ? cause.message : 'Your photo changes could not be saved. Try again.');
            return false;
        }
        finally {
            jobs.current -= 1;
            if (active.current)
                setBusy(jobs.current > 0);
        }
    };
    const load = async () => {
        ...
        if (saved && displayed.current === saved.id) {
            if (active.current)
                setDraft(saved);
            return;
        }
    ...
    useEffect(() => {
        ...
        const subscription = AppState.addEventListener('change', state => { if (state === 'active')
            void refresh(); });
        const changed = (event: StorageEvent) => { if (event.key === PHOTO_DRAFT_KEY || event.key === null)
            void refresh(); };
        if (Platform.OS === 'web')
        ...
        return () => {
            ...
            void latestJob.current.catch(() => { }).then(async () => {
                for (const id of pendingSources.current.keys())
                    await finishSource(id);
            }).catch(() => { });
        };
    }, []);
    const select = (source: string, name: string) => {
        ...
        let initialId: string | null | undefined;
        let committed = false;
        ...
        return run(async () => {
            ...
            if (initialId === undefined)
                initialId = current?.id || null;
            if (!committed && current?.id === next.id)
                committed = true;
            if (!committed) {
                if ((current?.id || null) !== initialId) {
                    await removePhotoFile(next.id);
                ...
                try {
                    await Storage.setItem(PHOTO_DRAFT_KEY, JSON.stringify({ version: 1, draft: next }));
                }
                catch (cause) {
                    const saved = await Storage.getItem(PHOTO_DRAFT_KEY);
                    if (!saved || parse(saved).id !== next.id) {
                        await removePhotoFile(next.id);
                        throw cause;
                    }
                }
            ...
            if (initialId && initialId !== next.id)
                await removePhotoFile(initialId);
            await finishSource(next.id);
            await load();
        });
    };
    const edit = (field: 'place' | 'moment' | 'style', value: string) => {
        ...
        return run(async () => {
            await Storage.updateItem(PHOTO_DRAFT_KEY, raw => {
                ...
                if (latest?.id !== id)
                    throw new Error('This photo draft changed in another window. Reopen it before editing.');
    ...
    const discard = () => {
        ...
        return run(async () => {
            ...
            if (raw !== null && raw !== expected) {
                ...
                try {
                    currentId = parse(raw).id;
                }
                catch { }
                if (!currentId || currentId !== expectedId)
                    throw new Error('This photo draft changed in another window. Reopen it before discarding.');
            }
            for (const id of pendingSources.current.keys())
                await finishSource(id);
            await removeAllPhotoFiles();
            await Storage.removeItem(PHOTO_DRAFT_KEY);
            await load();
        });
    };
