...
export function useProfilePickerRecovery(accountReady: boolean, accountId: string | undefined, onRestore: () => void) {
    ...
    const retry = async () => {
        if (Platform.OS !== 'android' || !accountReady || !accountId)
            return;
        ...
        try {
            ...
            if (active())
                setError('Your saved profile changes could not be restored. Retry or discard these unsaved changes.');
        }
    };
    const discard = async () => {
        setError('');
        try {
            await clearProfilePicker();
            if (mounted.current) {
                setSession(null);
                setReady(true);
            }
            ...
            if (mounted.current)
                setError('Your saved changes could not be discarded. Please try again.');
        }
    };
    useEffect(() => {
