...
export function LocalVoicePreview({ voice, onPlay, libraryPlaying }: { voice: OwnVoice; onPlay: () => void; libraryPlaying: boolean }) {
  const [track, setTrack] = useState<AudioTrack | null>(null);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(true);
  const [attempt, setAttempt] = useState(0);
  const playback = useTrackPlayback(track);
  useEffect(() => { if (libraryPlaying) playback.player.pause(); }, [libraryPlaying]);
  useEffect(() => {
    let alive = true; let release = () => {};
    ...
    void (async () => {
      ...
      if (!alive) { file?.release(); return; }
      release = file?.release || (() => {});
      setTrack({ id: voice.id, title: voice.name, styles: '', lyrics: '', source: example?.source || file!.source, clip: voice.sample.clip });
    })().catch(() => { if (alive) setError('Your saved Voice sample could not be opened. Try again.'); }).finally(() => { if (alive) setLoading(false); });
