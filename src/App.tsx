...
function CreationApp() {
  ...
  const C = theme.colors; const A = styles(C); const disc = theme.dark ? '#FFFFFF08' : '#1010120F'; const menuInk = theme.dark ? C.ink : '#2E2E33';
  ...
  const [editor, setEditor] = useState<{ kind: EditorKind; entry: 'editor' | 'saved' | 'save' } | null>(null);
  ...
  const moveHook = (direction: number) => { if (!currentHook || !hooks.length) return; const index = hooks.findIndex(clip => clip.id === currentHook.id); setHookId(hooks[(index + direction + hooks.length) % hooks.length].id); };
  const [searchNotifications, setSearchNotifications] = useState(false);
  useEffect(() => {
    if (Platform.OS !== 'web' || !searchNotifications) return;
    const escape = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || event.isComposing || document.querySelector('[aria-modal="true"]')) return;
      event.preventDefault(); event.stopImmediatePropagation(); setSearchNotifications(false);
    };
  ...
  const pickerPending = useRef(false); const pickerEpoch = useRef(0); const mediaAlive = useRef(true);
  ...
  useEffect(() => { if (audioPlayback.status.playing) libraryPlayer.player.pause(); }, [audioPlayback.status.playing]);
  const hookOwnsPlayback = tab === 'hooks' && hookExpanded && !hookCreator && !hookCollectionId && !playerOpen;
  useEffect(() => { if (hookOwnsPlayback) { audioPlayback.player.pause(); libraryPlayer.player.pause(); } }, [hookOwnsPlayback, audioPlayback.status.playing, libraryPlayer.status.playing]);
  const incomingURL = Linking.useLinkingURL();
  useEffect(() => {
    if (!incomingURL) return;
    const params = Linking.parse(incomingURL).queryParams;
    if (typeof params?.hook === 'string') { const clip = decodeHook(params.hook); if (!clip) { setNotice('This Hook link could not be opened.'); return; } setIncomingHook(clip); setHookId(clip.id); setHookExpanded(true); setTab('hooks'); return; }
    if (typeof params?.profile === 'string' || typeof params?.creator === 'string') {
      ...
      if (!target) { setNotice('This profile link could not be opened.'); return; }
      setCreatorTarget(target); setCreatorOpen(true); setTab('search'); return;
    }
    const raw = Linking.parse(incomingURL).queryParams?.playlist;
    if (typeof raw !== 'string') return;
    const shared = decodeSharedPlaylist(raw);
    if (!shared) { setNotice('This shared playlist link could not be opened.'); return; }
    library.setShared(shared); setLibraryRoute('shared'); setTab('library');
  }, [incomingURL]);
  ...
  const selectAudio = (track: AudioTrack) => {
    setSongPicker(false); setAudio(track);
    if (track.styles || track.lyrics) {
      if (state.mode === 'Simple' && !state.lyrics.value && !state.styles.value) { dispatch({ type: 'edit', kind: 'lyrics', value: track.lyrics }); dispatch({ type: 'edit', kind: 'styles', value: track.styles }); }
      else setPendingAudio(track);
    }
  };
  useEffect(() => {
    let active = true;
    AsyncStorage.getItem(DRAFT_KEY).then(raw => {
      ...
      if (saved && ['Simple', 'Advanced'].includes(saved.mode) && typeof saved.prompt === 'string' && Array.isArray(saved.saved) && saved.lyrics && saved.styles) dispatch({ type: 'restore', draft: saved });
    }).catch(() => { if (active) { draftRestoreFailed.current = true; setStorageError(true); } }).finally(() => { if (active) setReady(true); });
    return () => { active = false; };
  }, []);
  useEffect(() => {
    if (!ready || draftRestoreFailed.current) return;
    const raw = JSON.stringify(state);
    if (raw === draftPersisted.current) return;
  ...
  useEffect(() => {
    const listener = BackHandler.addEventListener('hardwareBackPress', () => {
      if (voiceDetail) { setVoiceDetail(null); return true; }
      if (overlay) { setOverlay(null); return true; }
      if (searchNotifications) { setSearchNotifications(false); return true; }
      if (tab === 'hooks' && currentHook && (hookCollectionId || hookExpanded || hookCreator)) return false;
      if (tab === 'search' && creatorOpen) { setCreatorOpen(false); return true; }
      if (tab === 'hooks' && !currentHook) { setTab('create'); return true; }
      if (tab === 'search' && searchPhase !== 'discover') { setSearchQuery(''); setSearchFilter('All'); setSearchPhase('discover'); return true; }
      if (tab !== 'create' && (tab !== 'library' || libraryRoute === 'home')) { setTab('create'); return true; }
      return false;
    });
  ...
  const chooseDocument = async () => {
    try {
      ...
      if (!result.canceled) { const asset = result.assets[0]; selectAudio({ id: `upload-${Date.now()}`, title: asset.name, source: { uri: asset.uri }, styles: '', lyrics: '' }); setOverlay(null); }
    } catch { setNotice('The file could not be opened. Please try again.'); }
  };
  const chooseMedia = async (kind: MediaKind) => {
    if (pickerPending.current || media.busy || !media.ready) return;
    pickerPending.current = true; media.setPickerOpen(true); const selection = ++pickerEpoch.current; setPickingMedia(true); setOverlay(null);
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        if (active()) await media.select(asset.uri, asset.fileName || (kind === 'images' ? 'Selected image' : 'Selected video'), kind, active);
        else await releaseMediaSource(asset.uri);
      }
    } catch { if (active()) setNotice('The media library could not be opened. Please try again.'); }
    finally { pickerPending.current = false; media.setPickerOpen(false); if (mediaAlive.current) setPickingMedia(false); }
  };
  const clearDraft = async () => {
    if (media.busy || pickingMedia) return;
    if (!await media.discard()) return;
    dispatch({ type: 'clear' }); setAudio(null); setAudioEdit({ mode: 'Cover', start: 0 });
  };
  const reuseTrack = async (track: AudioTrack, withAudio: boolean, isActive: () => boolean = () => true) => {
    if (pickerPending.current || media.busy || !media.ready || !audioReady || !ready || draftRestoreFailed.current || !isActive()) return false;
    setNotice('');
    ...
    if (withAudio) {
      ...
      try {
        ...
        if (active()) setNotice(error instanceof Error ? error.message : 'The remix could not be saved. Try again.');
        return false;
      }
    } else {
      dispatch({ type: 'edit', kind: 'lyrics', value: track.lyrics }); dispatch({ type: 'edit', kind: 'styles', value: track.styles });
    }
    setVoiceDetail(null); setTab('create');
    return true;
  };
  ...
  const section = (kind: EditorKind) => {
    const h = state[kind]; const title = kind === 'lyrics' ? 'Lyrics' : 'Styles';
    return <View style={A.section} key={kind}>
      ...
      {collapsed[kind] ? <Label style={A.summary}>{kind === 'lyrics' && state.instrumental ? 'Instrumental track - Lyrics disabled' : h.value || (kind === 'lyrics' ? 'Write lyrics or a prompt' : 'Describe your style')}</Label> : <>
        ...
        <View style={[A.sectionFooter, kind === 'styles' && { marginTop: 5 }]}>
          ...
          {kind === 'lyrics' ? <Pressable accessibilityRole="checkbox" accessibilityLabel="Instrumental" aria-checked={state.instrumental} accessibilityState={{ checked: state.instrumental }} onPress={() => { dispatch({ type: 'instrumental' }); if (!state.instrumental) setCollapsed(c => ({ ...c, lyrics: true })); }} style={A.instrumentalTarget}><View style={[A.instrumental, state.instrumental && { backgroundColor: C.ink }]}><Icon name="check-circle" color={state.instrumental ? C.primary : C.ink} size={20} /><Label style={[A.sectionTitle, { fontSize: 14.75 }, state.instrumental && { color: C.surface }]}>Instrumental</Label></View></Pressable> : <>
  ...
  if (recoveryError) return <View style={A.loading}><Label accessibilityRole="alert">{recoveryError}</Label><Pressable accessibilityRole="button" accessibilityLabel="Retry profile recovery" onPress={() => { if (!account.ready) account.retry(); else void profileRecovery.retry(); }} style={{ padding: 16 }}><Label>Retry</Label></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Discard unsaved profile recovery" onPress={() => void profileRecovery.discard()} style={{ padding: 16 }}><Label>Discard unsaved changes</Label></Pressable></View>;
  if (!ready || !audioReady || !media.ready || !profileRecovery.ready) return <View style={A.loading}><ActivityIndicator accessibilityLabel="Loading saved draft" color={C.ink} /></View>;
  return <View style={A.desktop}><View style={A.app}>
    ...
    {photoOpen && <LightSurface><PhotoToSong inspiration={photoInspiration} canSave={library.ready} onClose={() => setPhotoOpen(false)} onCreate={async (template, draft, prompt, isActive) => { const track = await library.createSample(template, draft.place, draft.style, prompt, undefined, isActive, { inputs: { mode: 'Simple', prompt, instrumental: false }, media: { id: draft.id, name: draft.name, kind: 'images', origin: 'photo' } }); if (!track) return false; setTab('library'); setLibraryRoute('home'); return true; }} /></LightSurface>}
    {playerOpen && <LibraryPlayer player={libraryPlayer} library={library} community={community} author={account.profile.name} onClose={() => { if (tab === 'hooks' && hookExpanded) libraryPlayer.player.pause(); setPlayerOpen(false); }} onReuse={(track, withAudio) => { void reuseTrack(track, withAudio); }} />}
    {enhancement.dialog}
    ...
    {overlay === 'model' && <ThemedSurface><CreateModelMenu model={entitlements.data.model} onClose={() => setOverlay(null)} onSelect={model => { if (model !== 'v6-mini' && entitlements.data.plan === 'Free') setOverlay('model-offer'); else void entitlements.selectModel(model).then(() => setOverlay(null)).catch(() => setNotice('The model selection could not be saved. Try again.')); }} /></ThemedSurface>}
    ...
    {voiceDetail && <LightSurface><VoiceDetails voice={voiceDetail} creator={voiceDetail.local ? ownCreator : creatorById(voiceDetail.creatorId)!} ownVoice={ownVoices.voices.find(voice => voice.id === voiceDetail.id)} account={account} songs={library.tracks} player={libraryPlayer} onClose={() => setVoiceDetail(null)} onUse={() => { audioPlayback.player.pause(); libraryPlayer.player.pause(); dispatch({ type: 'voice', id: voiceDetail.id }); setVoiceDetail(null); setTab('create'); }} onCreator={() => { if (voiceDetail.local) { setVoiceDetail(null); setTab('profile'); return; } if (tab === 'profile') { setVoiceDetail(null); return; } setCreatorTarget(creatorById(voiceDetail.creatorId)!); setCreatorOpen(true); setVoiceDetail(null); setTab('search'); }} onSongOptions={setGlobalSongOptions} /></LightSurface>}
    {overlay === 'voice' && <LightSurface><Voice canCreate={ownVoices.ready} onCreate={async (track, isActive) => { const voice = await ownVoices.create(track, isActive); if (isActive()) { dispatch({ type: 'voice', id: voice.id }); setOverlay(null); setTab('create'); setNotice('Local Voice saved to your profile. No voice model was trained.'); } }} ownerName={account.profile.handle || account.profile.name} songs={library.songs} publicSongs={library.songs.filter(track => library.data.songs?.find(song => song.id === track.id)?.isPublic)} likedSongs={library.tracks.filter(track => library.data.likedIds?.includes(track.id))} onClose={() => setOverlay(null)} onUse={track => { selectAudio(track); setOverlay(null); setNotice('Voice sample attached as local audio. Voice cloning is not connected.'); }} /></LightSurface>}
...
const styles = (C: ThemeColors) => StyleSheet.create({
  ...
  simpleCard: { flex: 1, marginHorizontal: 16, borderWidth: 1, borderColor: C.surface === '#101012' ? '#FFFFFF20' : '#1010121A', borderRadius: 20, padding: 15, paddingBottom: 3 },  // +1dp border = 16 from the card edge
  ...
  chips: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', columnGap: 8, rowGap: 0, marginTop: -4 },  // 48dp touch rows touch; visible 40dp pills keep 8dp
  ...
  // rows are 48dp touch rows; the visible 40dp discs/pills sit 4dp inside, so margins absorb the difference against the 20dp card padding
  sectionHeader: { height: 48, marginTop: -4, marginLeft: -6, marginRight: -12, marginBottom: 15 },
  sectionFooter: { flexDirection: 'row', alignItems: 'center', height: 48, marginTop: 21, marginBottom: -5, marginHorizontal: -5 },  // +1/-1 keeps card height; text line sits 1dp lower than the row
