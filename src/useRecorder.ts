...
export function useRecorder(minimumSeconds = 6) {
  ...
  const start = async () => {
    if (pending.current || recorder.isRecording) return;
    pending.current = true; setBusy(true); setError('');
    try {
      const permission = await requestRecordingPermissionsAsync();
      if (!alive.current) return;
      if (!permission.granted) throw new Error('Microphone access was denied. Allow it in device settings, or upload an audio file.');
      await setAudioModeAsync({ allowsRecording: true, playsInSilentMode: true });
      await recorder.prepareToRecordAsync();
      if (!alive.current) return;
      recorder.record();
    } catch (cause) { if (alive.current) setError(cause instanceof Error ? cause.message : 'The microphone could not be opened.'); }
  ...
  const stop = async () => {
    ...
    try {
      ...
      if (!alive.current) return;
      if (duration < minimumSeconds) { setError(`Audio too short. Record at least ${minimumSeconds} seconds.`); return; }
      if (!recorder.uri) throw new Error('No audio was recorded. Please try again.');
      setFile({ uri: recorder.uri, duration });
    } catch (cause) { if (alive.current) setError(cause instanceof Error ? cause.message : 'Recording could not be saved.'); }
    finally { pending.current = false; if (alive.current) setBusy(false); }
  };
