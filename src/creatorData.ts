...
export type ExampleCreator = { id: string; name: string; handle: string; avatar: ImageSourcePropType; cover: ImageSourcePropType; bio?: string; tracks: AudioTrack[]; shared?: boolean };
...
export function decodeSharedProfile(raw: string): ExampleCreator | null {
  ...
  try {
    ...
    if (value?.version !== 1 || typeof value.id !== 'string' || !/^[a-zA-Z0-9_-]{1,100}$/.test(value.id) || typeof value.name !== 'string' || !value.name.trim() || value.name.length > 60 || typeof value.handle !== 'string' || !/^[a-zA-Z0-9_-]{3,30}$/.test(value.handle) || typeof value.bio !== 'string' || value.bio.length > 200) return null;
