...
export function TextEditor({ kind, state, dispatch, onClose, entry = 'editor' }: {
    ...
    const save = () => {
        if (!canSaveName || !naming)
            return;
        if (naming.type === 'rename')
            dispatch({ type: 'rename', id: naming.item.id, name });
        else
            dispatch({ type: 'save', item: { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, kind, name: name.trim(), value: h.value, createdAt: Date.now() } });
        setNaming(null);
        Keyboard.dismiss();
        if (entry === 'save')
            onClose();
    };
    const cancelName = () => { setNaming(null); Keyboard.dismiss(); if (entry === 'save')
        onClose(); };
    return <>
    {entry !== 'save' && <Sheet onClose={close} onShow={() => { if (entry === 'editor')
            input.current?.focus(); }}>
    {listing ? <View ref={listingView} collapsable={false} style={{ flex: 1 }}>
      ...
      <ScrollView onScrollBeginDrag={() => setMenu(null)} contentContainerStyle={[E.list, !items.length && { flexGrow: 1, justifyContent: 'center' }]}>
        ...
        {items.map(item => <View key={item.id} style={E.savedRow}>
          <Pressable accessibilityRole="button" accessibilityLabel={`Use ${item.name}`} onPress={() => { dispatch({ type: 'edit', kind, value: item.value }); if (entry === 'saved')
                    onClose();
                else
                    setListing(false); }} style={E.savedContent}>
          ...
          <View collapsable={false} ref={view => { if (view)
                    anchors.current.set(item.id, view);
                else
      ...
      <TextInput ref={input} multiline autoFocus={Platform.OS === 'web'} accessibilityLabel={title} value={h.value} onChangeText={value => dispatch({ type: 'edit', kind, value })} style={E.input} placeholder={kind === 'lyrics' ? 'Write your lyrics here' : 'Describe what you want your song to sound like'} placeholderTextColor={C.placeholder} selectionColor={C.primary} textAlignVertical="top"/>
      <View style={E.footer}>
...
const styles = (C: ThemeColors) => StyleSheet.create({
    ...
    footer: { flexDirection: 'row', alignItems: 'center', paddingVertical: 16, paddingLeft: 16, paddingRight: 14, gap: 7 },
