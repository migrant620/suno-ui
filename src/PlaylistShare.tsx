...
export function PlaylistShare({ playlist, tracks, onClose }: {
    playlist: Playlist;
    tracks?: AudioTrack[];
    onClose: () => void;
}) {
    ...
    const share = async (action: string) => {
        ...
        try {
            ...
            if (action === 'Copy Link') {
                ...
                await Linking.openURL(`mailto:?subject=${encodeURIComponent(playlist.name)}&body=${encodeURIComponent(message)}`);
            else if (Platform.OS === 'web' && typeof navigator.share !== 'function') {
                await Clipboard.setStringAsync(url);
                setNotice('Link copied. Paste it to share this example playlist.');
            }
            else
                await Share.share({ title: playlist.name, message, ...(Platform.OS === 'ios' ? { url } : {}) });
        }
        catch (error) {
    ...
    <View style={P.options}>{([['Copy Link', 'link-variant'], ['Messages', 'message'], ['Email', 'email'], ['More', 'dots-vertical']] as [
        string,
        IconName
    ][]).map(([name, icon]) => <Pressable key={name} accessibilityRole="button" accessibilityLabel={name} disabled={busy} onPress={() => void share(name)} style={P.option}><View style={P.icon}><Icon name={icon} size={24} color="#333333"/></View><Label style={P.label}>{name}</Label></Pressable>)}</View>
