...
export function PlaylistDetails({ item, liked, shared = false, tracks, player, onBack, onCreate, onAdd, onMenu, onSongOptions, backLabel = 'Back to Library', curated }: {
    ...
    {sortOpen && <View style={StyleSheet.absoluteFill}><Pressable accessibilityRole="button" accessibilityLabel="Close song sort" onPress={() => setSortOpen(false)} style={StyleSheet.absoluteFill}/><View style={D.sortMenu}>{playlistSorts.map(value => <Pressable accessibilityRole="button" key={value} onPress={() => { setSort(value); if (value === 'Default')
        setSortOpen(false); }} style={D.sortRow}><View style={{ width: 24 }}>{sort === value && <Icon name="check" size={20}/>}</View><Label style={{ fontSize: 16 }}>{value}</Label></Pressable>)}{sort !== 'Default' && <><View style={D.separator}/>{(['Ascending', 'Descending'] as const).map(value => <Pressable key={value} accessibilityRole="button" onPress={() => { setDirection(value); setSortOpen(false); }} style={D.sortRow}><View style={{ width: 24 }}>{direction === value && <Icon name="check" size={20}/>}</View><Label>{value}</Label></Pressable>)}</>}</View></View>}
