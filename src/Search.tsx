...
function SearchContent({ history, library, player, query, setQuery, filter, setFilter, phase, setPhase, scrollOffsets, onSongOptions, onProfile, onPlaylist, onInspiration, onNotifications, onOpenPlayer, commentCounts }: {
  history: ReturnType<typeof useSearchHistory>; library: ReturnType<typeof useLibrary>; player: ReturnType<typeof useLibraryPlayback>;
  query: string; setQuery: (query: string) => void; filter: string; setFilter: (filter: string) => void;
  phase: SearchPhase; setPhase: (phase: SearchPhase) => void; scrollOffsets: React.MutableRefObject<Record<string, number>>;
  onSongOptions: (song: AudioTrack) => void; onProfile: (creator?: ExampleCreator) => void; onPlaylist: (id: string) => void;
  commentCounts: Record<string, number>; onInspiration: (kind: 'photo' | 'record') => void; onNotifications: () => void; onOpenPlayer: () => void;
}) {
  const C = useSurfaceColors(); const S = styles(C);
  ...
  const submit = () => { if (search) { setPhase('results'); setFilter('All'); } };
