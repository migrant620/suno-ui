...
export function ProfileEditor({ account, onClose, origin = 'profile', recovery }: { account: ReturnType<typeof useLocalProfile>; onClose: () => void; origin?: 'profile' | 'settings'; recovery?: ProfilePickerSession | null }) {
  ...
  const close = async () => {
    ...
    closing.current = true; generation.current += 1;
    if (session.current) {
      setBusy(true);
      try { await clearProfilePicker(session.current.id); session.current = null; }
      catch { closing.current = false; setBusy(false); setError('Your changes could not be discarded. Please try again.'); return; }
    }
    alive.current = false; Keyboard.dismiss(); onClose();
  };
  const modalRoot = useRef<View>(null);
  useSheetEscape(close, modalRoot);
  const choose = async (kind: 'avatar' | 'cover', camera = false) => {
    if (picking.current || !alive.current || !account.ready) return;
    ...
    try {
      ...
      if (camera && Platform.OS !== 'web') {
        ...
        if (!active()) return;
        if (!permission.granted) { setError('Camera access was not granted. Choose from Gallery or allow camera access in your device settings.'); return; }
      }
      ...
      if (!result.canceled && active()) {
        ...
        if (session.current && active()) {
          ...
          await persistProfilePicker(next, active); session.current = next;
        }
        if (active()) { if (kind === 'avatar') setAvatar(uri); else setCover(uri); }
      }
    } catch { if (active()) setError('This image could not be saved. Try again or choose another image.'); }
    finally {
      if (session.current?.pending && active()) {
        const next = { ...session.current, pending: false };
        try { await persistProfilePicker(next, active); session.current = next; }
        catch { if (active()) setError('Your photo changes could not be retained. Please try again.'); }
      }
      picking.current = false; if (active()) setBusy(false);
    }
  };
  const save = async () => {
    if (!canSave || saving.current || !alive.current) return;
    saving.current = true; setBusy(true); setSubmitting(true); setError('');
    const attempt = ++generation.current;
    const active = () => alive.current && !closing.current && generation.current === attempt;
    ...
    if (active()) {
      ...
      if (saved) { session.current = null; void close(); }
      else setError('Profile changes could not be saved. Please try again.');
    }
  };
