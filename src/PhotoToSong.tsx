...
export function PhotoToSong({ onClose, canSave, onCreate, inspiration = 'photo' }: {
    inspiration?: 'photo' | 'love';
    onClose: () => void;
    canSave: boolean;
    onCreate: (template: string, draft: PhotoDraft, prompt: string, isActive: () => boolean) => Promise<boolean>;
}) {
    ...
    const [editor, setEditor] = useState<Field | null>(null);
    ...
    useEffect(() => {
        const listener = AppState.addEventListener('change', state => { setForeground(state === 'active'); if (state === 'active')
            void getPermission();
        else
    ...
    useEffect(() => {
        if (Platform.OS === 'web' || !photo.ready || photo.draft || photo.error || !permission || asked.current)
            return;
        asked.current = true;
        if (!permission.granted && permission.canAskAgain)
            void requestPermission().catch(() => setCameraError('Camera access could not be requested. Try again.'));
    ...
    const allow = async () => {
        ...
        try {
            ...
            setCameraError('Camera access could not be requested. Try again.');
        }
    };
    const accept = async (uri: string, name: string) => {
        if (!alive.current) {
            await releasePhotoSource(uri);
            return;
        }
        setOpening(true);
        try {
            ...
            if (alive.current)
                setError(cause instanceof Error ? cause.message : 'The image could not be opened. Try again.');
        }
        finally {
            if (alive.current)
                setOpening(false);
    ...
    const gallery = async () => {
        ...
        try {
            ...
            if (!result.canceled && result.assets[0])
                await accept(result.assets[0].uri, result.assets[0].fileName || 'Selected photo');
        }
        catch {
            if (alive.current)
    ...
    const capture = async () => {
        ...
        if (pending.current || !cameraReady || photo.busy)
            return;
        pending.current = true;
        setError('');
        setOpening(true);
        try {
            ...
            if (!picture)
                throw new Error('The photo could not be captured. Try again.');
    ...
    const startEdit = (field: Field) => { if (!photo.draft || photo.busy)
        return; setEditor(field); setValue(photo.draft[field]); };
    const commit = async () => { if (!editor)
        return true; const field = editor; const saved = await photo.edit(field, value.trim()); if (saved) {
        setEditor(null);
        Keyboard.dismiss();
    } return saved; };
    const cycle = (field: 'moment' | 'style') => {
        if (!photo.draft || photo.busy)
            return;
    ...
    const cameraOpened = () => {
        if (!alive.current || mountedCamera.current !== cameraIdentity)
            return;
        workingFacing.current = facing;
        setCameraReady(true);
    };
    const cameraFailed = () => {
        if (!alive.current || mountedCamera.current !== cameraIdentity)
            return;
        mountedCamera.current = null;
        setCameraReady(false);
        if (workingFacing.current && workingFacing.current !== facing) {
            setCameraKey(n => n + 1);
            setFacing(workingFacing.current);
        }
        else
            setCameraError('The camera could not be opened. Try again or choose a photo.');
    };
    const switchCamera = () => {
        if (pending.current || photo.busy || !cameraReady)
            return;
        mountedCamera.current = null;
        setCameraReady(false);
        setCameraKey(n => n + 1);
    ...
    useEffect(() => { if (!cameraVisible)
        setCameraReady(false); }, [cameraVisible]);
    ...
    {!photo.ready ? <View style={P.center}><ActivityIndicator color="#101012" accessibilityLabel="Loading photo draft"/></View> : photo.draft ? <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
      ...
      <View style={P.footer}>{editor && <Pressable accessibilityRole="button" accessibilityLabel="Save photo word" disabled={photo.busy} onPress={() => void commit()} style={P.editDone}><Label style={P.editDoneText}>Done</Label></Pressable>}{!editor && <Label style={P.hint}>Tap the pink words to cycle options or long press to edit</Label>}<Label style={P.disclosure}>Local prompt template · No image analysis or new music generation</Label><Pressable accessibilityRole="button" accessibilityLabel="Create photo song" disabled={!valid} accessibilityState={{ disabled: !valid }} onPress={() => { void commit().then(saved => { if (saved && alive.current)
            setExample(true); }); }}>{valid ? <Accent style={P.create}><Icon name="music-note" color="#FFFFFF"/><Label style={P.createText}>Create song</Label></Accent> : <View style={[P.create, { backgroundColor: '#4D4D4D' }]}><Icon name="music-note" color="#AAA8A4"/><Label style={[P.createText, { color: '#AAA8A4' }]}>Create song</Label></View>}</Pressable></View>
    ...
    {example && photo.draft && <ExampleCreation mediaName={photo.draft.name} title={photo.draft.place} ready={canSave} onClose={() => setExample(false)} onCreate={async (template, isActive) => { if (await onCreate(template, photo.draft!, prompt, isActive)) {
        setExample(false);
        onClose();
    } }}/>}
