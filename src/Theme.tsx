...
type ThemeValue = {
    ...
    ready: boolean;
    error: string;
    select: (mode: ThemeMode) => Promise<boolean>;
    retry: () => void;
};
...
export function ThemedSurface({ children }: {
    children: React.ReactNode;
}) { const theme = useTheme(); return <Surface.Provider value={theme.colors}>{children}</Surface.Provider>; }
export function LightSurface({ children }: {
    children: React.ReactNode;
}) { return <Surface.Provider value={light}>{children}</Surface.Provider>; }
export function ThemeProvider({ children }: {
    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem('suno-ui:theme:v1').then(value => {
            ...
            if (current) {
                setMode((value as ThemeMode) || 'system');
                setReady(true);
                setError('');
            }
        }).catch(() => { if (current)
            setError('Your theme could not be loaded. Try again.'); });
    ...
    const select = (next: ThemeMode) => {
        ...
        const job = writes.current.catch(() => { }).then(async () => {
            if (!ready)
                return;
            try {
                await AsyncStorage.setItem('suno-ui:theme:v1', next);
                saved = true;
    ...
    return <Theme.Provider value={{ mode, dark: isDark, colors: isDark ? dark : light, ready, error, select, retry: () => { if (failedMode.current)
            void select(failedMode.current);
        else
            setLoadAttempt(value => value + 1); } }}>{children}</Theme.Provider>;
}
