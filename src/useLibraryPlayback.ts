...
export function useLibraryPlayback(onStart: () => void) {
    ...
    const start = async (song: AudioTrack, songs: AudioTrack[], id: string) => {
        ...
        try {
            ...
            release.current();
            release.current = offline?.release || (() => { });
            setTrack(song);
            setQueue(songs);
            setPlaylistId(id);
            ...
            if (version === request.current)
                playback.setError('This song could not be played. Try again.');
        }
        finally {
            if (version === request.current)
                setLoading(false);
    ...
    const playTrack = async (song: AudioTrack, songs: AudioTrack[], id: string) => {
        if (song.id === track?.id && id === playlistId)
            await toggle();
        else
            await start(song, songs, id);
    };
    const next = (automatic = false) => {
        const index = queue.findIndex(song => song.id === track?.id);
        if (automatic && repeat === 'one' && track) {
            void start(track, queue, playlistId);
            return;
        }
        ...
        else if (index >= 0 && index + 1 < queue.length)
            void start(queue[index + 1], queue, playlistId);
        else if (repeat === 'all' && queue.length)
            void start(queue[0], queue, playlistId);
    };
    const previous = () => {
        const index = queue.findIndex(song => song.id === track?.id);
        if (playback.current > 3 || index <= 0)
            void playback.seek(0);
        else
            void start(queue[index - 1], queue, playlistId);
    };
    useEffect(() => { if (playback.status.didJustFinish)
        next(true); }, [playback.status.didJustFinish]);
