...
type Props = {
    ...
    onCreate: (track: AudioTrack, isActive: () => boolean) => Promise<void>;
    canCreate: boolean;
};
...
export function Voice({ ownerName, songs, publicSongs, likedSongs, onClose, onUse, onCreate, canCreate }: Props) {
    ...
    useEffect(() => {
        if (!recorder.file || recorder.file.uri === lastRecording.current)
            return;
    ...
    useEffect(() => {
        if (route !== 'loading' || !selected || waveform.loading)
            return;
        if (waveform.error) {
            ...
            if (waveform.duration < 15) {
                setError('This audio is too short. Audio must be at least 15 seconds.');
                setSelected(null);
                recorder.discard();
                setRoute('main');
            }
            else {
                setRange({ start: 0, end: waveform.duration });
                setRoute('trim');
            }
        }
    }, [route, selected, waveform]);
    useEffect(() => { if (recorder.recording && stopAfterStart.current) {
        stopAfterStart.current = false;
        void recorder.stop();
    } }, [recorder.recording]);
    const start = async () => {
        if (starting.current || recorder.busy)
            return;
        if (!explained) {
            setPermission(true);
            hold.current = false;
    ...
    const upload = async () => {
        ...
        try {
            ...
            if (!alive.current || result.canceled)
                return;
            ...
            if (!alive.current)
                return;
            if (info && !info.exists)
                throw new Error('The audio file could not be opened. Try again.');
            ...
            if (alive.current)
                setError(cause instanceof Error ? cause.message : 'The audio file could not be opened. Try again.');
        }
        finally {
    ...
    const createVoice = async () => {
        if (!selected || !usable || !canCreate || savePending.current)
            return;
        ...
        try {
            ...
            if (isActive())
                setError(cause instanceof Error ? cause.message : 'Your Voice could not be saved. Try again.');
        }
        finally {
    ...
    const finish = () => { if (!selected || !usable)
        return; playback.player.pause(); if (selected.id.startsWith('voice-recording-'))
        recorder.take(); onUse({ ...selected, title: voiceName.trim() || `${ownerName}'s voice`, clip: range, waveform: waveform.peaks.slice(Math.floor(range.start / waveform.duration * waveform.peaks.length), Math.ceil(range.end / waveform.duration * waveform.peaks.length)) }); };
    ...
    {route === 'main' || route === 'loading' ? <View style={[V.main, { paddingBottom: bottom }]}>
      ...
      <View style={V.controls}>
        ...
        } }} onPress={() => { if (starting.current || stopAfterStart.current)
            return; if (recorder.recording) {
            void recorder.stop();
            return;
        } const now = Date.now(); if (now - lastTap.current < 350) {
            lastTap.current = 0;
            void start();
        }
        else
            lastTap.current = now; }} onAccessibilityTap={() => recorder.recording ? void recorder.stop() : void start()} style={({ pressed }) => [V.record, pressed && V.pressed]}>
