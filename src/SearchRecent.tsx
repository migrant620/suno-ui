...
export function SearchRecent({ history, library, onCreator, onSong, onPlaylist }: {
  history: ReturnType<typeof useSearchHistory>; library: ReturnType<typeof useLibrary>;
  onCreator: (creator: ExampleCreator) => void; onSong: (song: AudioTrack) => void; onPlaylist: (id: string) => void;
}) {
  const colors = useSurfaceColors(); const S = styles(colors);
  const rows = history.entries.map(item => {
    if (item.kind === 'creator') { const creator = creatorById(item.id); return creator && { item, title: creator.name, caption: `Creator · @${creator.handle} · Local example`, image: creator.avatar, open: () => onCreator(creator) }; }
    if (item.kind === 'song') { const song = library.tracks.find(track => track.id === item.id); return song && { item, title: song.title, caption: `Song · ${song.artist || 'Local example'}`, image: song.cover, open: () => onSong(song) }; }
  ...
  useEffect(() => { if (history.ready && library.ready && !history.busy && !history.error && missing.length) history.remove(missing); }, [missingKey, history.ready, library.ready, history.busy, history.error]);
