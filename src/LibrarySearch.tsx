...
export function LibrarySearch({ library, player, onSongOptions, onBack }: {
    ...
    const tracks = search ? library.tracks.filter(track => (library.songs.some(song => song.id === track.id) || library.data.likedIds?.includes(track.id) || library.data.playlists.some(item => item.songIds.includes(track.id))) &&
        `${track.title} ${track.styles} ${track.artist}`.toLowerCase().includes(search)) : [];
    useEffect(() => {
        if (Platform.OS !== 'web')
            return;
        const escape = (event: KeyboardEvent) => {
            if (event.key === 'Escape' && !event.isComposing && !document.querySelector('[role="dialog"],[aria-modal="true"]')) {
    ...
    <View testID="library-search-header" style={S.header}>
      ...
      <View testID="library-search-field" style={S.search}>
        ...
        {!!query && <Pressable accessibilityRole="button" accessibilityLabel="Clear search" onPress={() => { setQuery(''); if (Platform.OS === 'web')
        input.current?.focus(); }} style={({ pressed }) => [S.clear, pressed && { opacity: 0.65 }]}><X color={C.muted} size={24} strokeWidth={2}/></Pressable>}
      </View>
    </View>
