...
async function hasOfflinePlaylistRaw(playlistId: string, tracks: AudioTrack[]): Promise<boolean> {
  if (!tracks.length || !('caches' in globalThis) || !(await caches.has(cacheName(playlistId)))) return false;
...
async function saveOfflinePlaylistRaw(playlistId: string, tracks: AudioTrack[], signal: AbortSignal): Promise<void> {
  if (!('caches' in globalThis)) throw new Error('Offline storage is unavailable in this browser.');
  const cache = await caches.open(cacheName(playlistId));
  for (const track of tracks) {
    ...
    if (!blob.size) throw new Error('The downloaded audio is empty.');
...
async function removeOfflinePlaylistRaw(playlistId: string): Promise<void> {
  if ('caches' in globalThis) await caches.delete(cacheName(playlistId));
}
async function offlineTrackSourceRaw(playlistId: string, track: AudioTrack): Promise<{ source: AudioTrack['source']; release: () => void } | null> {
  if (!('caches' in globalThis) || !(await caches.has(cacheName(playlistId)))) return null;
