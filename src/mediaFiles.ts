        ...
        await new Promise<void>((resolve, reject) => {
            ...
            const listener = player.addListener('statusChange', event => {
                if (event.status === 'readyToPlay')
                    ...
                    finish(new Error('The video could not be opened. Choose another file.'));
            });
...
export async function saveMediaFile(id: string, uri: string, kind: MediaKind) {
    ...
    if (!info.exists || info.isDirectory || !info.size)
        throw new Error('The media file could not be saved.');
}
export async function readMediaFile(id: string, kind: MediaKind) {
    const uri = path(id, kind);
...
export async function releaseMediaSource(uri: string) {
    ...
    if (!FileSystem.cacheDirectory || !uri.startsWith(prefix) || !/^[a-z0-9-]+\.(png|jpe?g|heic|webp|mp4|mov|m4v|webm)$/i.test(uri.slice(prefix.length)))
        return;
    await FileSystem.deleteAsync(uri, { idempotent: true });
}
