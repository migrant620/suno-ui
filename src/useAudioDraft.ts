...
type SavedAudio = {
    track: Omit<AudioTrack, 'source' | 'cover'>;
    edit: AudioEdit;
};
export function useAudioDraft() {
    ...
    useEffect(() => {
        ...
        void withOfflineLock(key, async () => {
            ...
            if (typeof saved?.track?.id !== 'string' || !saved.track.id || typeof saved.track.title !== 'string' || !['Cover', 'Extend'].includes(saved.edit?.mode) || !Number.isFinite(saved.edit.start) || saved.edit.start < 0)
                throw new Error('Invalid audio draft');
            ...
            setError('Your saved audio could not be restored. Select the file again to retry.');
        } })
            .finally(() => { if (active)
            setReady(true); });
    ...
    useEffect(() => {
        if (!ready || (restorationFailed.current && !audio))
            return;
        ...
        void withOfflineLock(key, async () => {
            ...
            if (audio) {
                if (audio.id !== oldId && !exampleTracks.some(track => track.id === audio.id || track.id === audio.templateId))
                    await saveDraftAudioFile(audio.id, audio.source);
            ...
            restorationFailed.current = false;
            if (oldId && oldId !== audio?.id && !exampleTracks.some(track => track.id === oldId))
                await removeDraftAudioFile(oldId);
            if (active)
                setError('');
        }).catch(() => { if (active)
            setError('Audio changes could not be saved on this device. Keep this page open and try again.'); });
        return () => { active = false; };
    }, [audio, audioEdit, ready]);
    const discardUnavailableAudio = async () => {
        if (!restorationFailed.current || discarding)
            return;
        setDiscarding(true);
        try {
            await withOfflineLock(key, async () => {
                if (!restorationFailed.current)
                    return;
                const id = stored.current;
                if (id && !exampleTracks.some(track => track.id === id))
                    await removeDraftAudioFile(id);
                await AsyncStorage.removeItem(key);
                stored.current = null;
                restorationFailed.current = false;
            });
