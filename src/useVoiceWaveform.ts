...
export function useVoiceWaveform(track: AudioTrack | null): VoiceWaveform {
    ...
    useEffect(() => {
        ...
        if (!track) {
            setState({ peaks: [], duration: 0, loading: false, error: '' });
            return;
        }
        if (track.waveform?.length && track.duration) {
            setState({ peaks: track.waveform, duration: track.duration, loading: false, error: '' });
            return;
        }
        ...
        if (!uri || !NativeModules.SunoAudio) {
            setState({ peaks: [], duration: 0, loading: false, error: 'Audio waveform decoding is unavailable on this platform.' });
            return;
        }
        NativeModules.SunoAudio.decodeWaveform(uri, 64, id).then((result: {
            peaks: number[];
            duration: number;
        }) => {
            if (active)
            ...
            setState({ peaks: [], duration: 0, loading: false, error: 'This audio could not load. Choose another sample.' }); });
