...
export const CameraView = forwardRef<CameraView, CameraViewProps>(function PhotoCamera({ facing = 'back', style, onCameraReady, onMountError }, ref) {
    ...
    useEffect(() => {
        ...
        const fail = () => {
            if (canceled || failed)
                return;
            failed = true;
            callbacks.current.onMountError?.({ message: 'The camera could not be opened.' });
        };
        const ready = () => {
            if (!canceled && !failed && acquired && element && element.readyState >= 2)
    ...
    useImperativeHandle(ref, () => ({
        async takePictureAsync(options = {}) {
            const element = video.current;
            if (!element || element.readyState < 2 || !element.videoWidth || !element.videoHeight || !stream.current?.getVideoTracks().some(track => track.readyState === 'live'))
                throw new Error('The camera is not ready. Try again.');
