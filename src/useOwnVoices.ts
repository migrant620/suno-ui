...
export type OwnVoice = PublicVoice & {
    local: true;
    sample: {
        fileId?: string;
        templateId?: string;
...
function parse(raw: string | null): OwnVoice[] {
    ...
    if (!Array.isArray(values) || !values.every(v => v?.local === true && /^local-voice-[a-z0-9-]+$/.test(v.id) && v.creatorId === 'local-owner' && typeof v.name === 'string' && v.name.trim() && v.name.length <= 100 && v.trial === false && Number.isFinite(v.sample?.clip?.start) && v.sample.clip.start >= 0 && Number.isFinite(v.sample.clip.end) && v.sample.clip.end - v.sample.clip.start >= 14.99 && (v.sample.fileId === v.id || exampleTracks.some(t => t.id === v.sample.templateId))))
        throw new Error('Your saved Voices could not be read.');
    return values;
}
export function useOwnVoices() {
    ...
    useEffect(() => {
        let current = true;
        setReady(false);
        void Storage.getItem(key).then(raw => { if (current) {
            setVoices(parse(raw));
            setError('');
            setReady(true);
        } }).catch(() => { if (current)
            setError('Your saved Voices could not be read. Retry to restore them.'); });
        return () => { current = false; };
    }, [attempt]);
    const create = async (track: AudioTrack, isActive: () => boolean): Promise<OwnVoice> => {
        if (!ready || !track.clip || track.clip.end - track.clip.start < 14.99)
            throw new Error('Choose at least 15 seconds of audio.');
        ...
        return withOfflineLock(key, async () => {
            ...
            let committed = false;
            try {
                if (!isActive() || !alive.current)
                    throw new Error('Voice creation canceled.');
                if (!template) {
                    copied = true;
                    await saveDraftAudioFile(id, track.source);
                }
                if (!isActive() || !alive.current)
                    throw new Error('Voice creation canceled.');
