...
export function SettingsFlow({ account, entitlements, onClose, recovery, onRecoveryClosed }: { recovery?: ProfilePickerSession | null; onRecoveryClosed?: () => void; account: ReturnType<typeof useLocalProfile>; entitlements: ReturnType<typeof useDemoEntitlements>; onClose: () => void }) {
  const theme = useTheme(); const C = theme.colors; const S = styles(C); const session = useLocalSession();
  ...
  const back = () => { if (busy) return; if (deletion) setDeletion(false); else if (offer) setOffer(false); else if (modelMenu) setModelMenu(false); else if (route === 'report' && reportPhase !== 'edit') setReportPhase('edit'); else if (route !== 'settings') changeRoute('settings'); else onClose(); };
  ...
  const chooseModel = async (model: MusicModel) => {
    if (!entitlements.ready || busy) return;
    setModelMenu(false); setError('');
    if (model !== 'v6-mini' && entitlements.data.plan === 'Free') { setOffer(true); return; }
    setBusy(true);
    try { await entitlements.selectModel(model); } catch { setError('The model could not be saved. Select it again to retry.'); } finally { setBusy(false); }
  };
  const language = async () => {
    changeRoute('language');
    if (Platform.OS !== 'android') return;
    setBusy(true);
    try { await IntentLauncher.startActivityAsync('android.settings.APP_LOCALE_SETTINGS', { data: 'package:dev.m620.sunoui' }); }
    catch { setError('App language settings could not be opened. Try again.'); }
    finally { setBusy(false); }
  };
  const saveReport = async () => {
    if (busy || !report.trim()) return;
    setBusy(true); setError('');
    try { await AsyncStorage.setItem('suno-ui:problem-report:v1', JSON.stringify({ text: report.trim(), savedAt: new Date().toISOString() })); setSavedReport(report.trim()); setReportPhase('saved'); }
    catch { setError('The report could not be saved on this device. Try again.'); }
    finally { setBusy(false); }
  };
  const openReport = async () => {
    changeRoute('report'); setReportPhase('edit'); setBusy(true);
    try { const raw = await AsyncStorage.getItem('suno-ui:problem-report:v1'); if (raw) { const value = JSON.parse(raw); if (typeof value.text !== 'string') throw new Error('Invalid report'); setReport(value.text); setSavedReport(value.text); } }
    catch { setError('Your last local report could not be loaded. Reopen this page to retry.'); }
    finally { setBusy(false); }
  };
