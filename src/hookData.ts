...
export function decodeHook(raw: string): ExampleHook | null {
    ...
    try {
        ...
        const template = exampleHooks.find(clip => clip.track.id === value.template);
        if (value?.version !== 1 || !template || typeof value.id !== 'string' || !/^[a-zA-Z0-9_-]{1,100}$/.test(value.id) || typeof value.title !== 'string' || !value.title.trim() || value.title.length > 200 || typeof value.caption !== 'string' || value.caption.length > 500 || typeof value.creator !== 'string' || value.creator.length > 60)
            return null;
